...
const API_BASE = "http://localhost:9000/api".replace(/\/+$/, "");
...
export async function closePayrollV2(payload: { year: number; month: number; bankAcc: string; salaryExpenseAcc: string; note?: string }): Promise<any> {
  ...
  const res = await fetch(url, { method: 'POST', headers: authHeaders(), body: JSON.stringify({ ...payload, locked: true }) });
...
export async function createV2Loan(payload: { employeeId: number; principal: number; startYear: number; startMonth: number; currency?: string; monthlyPercent?: number; customMonthlyPayment?: number; capMultiple?: number; note?: string }): Promise<any> {
