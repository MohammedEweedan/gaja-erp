...
const LeaveRequestScreen: React.FC<
  { employeeId?: number | string; isAdmin?: boolean } & SidebarAdaptive
> = ({
  employeeId,
  isAdmin = false,
  sidebarOpen,
  ...
  const [allowPastDates, setAllowPastDates] = useState(isAdmin);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isAdmin && allowPastDates) {
        payload.isAdminOverride = true;
        payload.adminOverrideReason = adminOverrideReason.trim();
        payload.overridePastDates = true;
      }
                    ...
                    <Box sx={{ mb: 2 }}>
                      ...
                      {allowPastDates && (
                        <TextField
                          ...
                          placeholder="Reason for creating past vacation..."
                          multiline
                          rows={2}
                          fullWidth
                          required={allowPastDates}
                        />
                      )}
                    </Box>
                  )}
                  ...
                  >
                    <Button
                      ...
                      disabled={
                        ...
                        !formData.leaveCode ||
                        !!daysError ||
                        (allowPastDates && !adminOverrideReason.trim())
                      }
                      startIcon={
                        submitting ? (
