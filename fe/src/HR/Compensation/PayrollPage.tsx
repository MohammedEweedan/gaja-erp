...
export default function PayrollPage() {
  ...
  // Session mode state
  const [sessionMode, setSessionMode] = React.useState<boolean>(false);
  const [currentEmployeeIndex, setCurrentEmployeeIndex] = React.useState<number>(0);
  const [processedEmployees, setProcessedEmployees] = React.useState<Set<number>>(new Set());
  ...
  // Allow USD/LYD reimbursements for any employee regardless of base salary currency
  const adjUsdEligible = React.useMemo(() => {
    return !!adjEmpId; // Any selected employee can have USD or LYD adjustments
  }, [adjEmpId]);
  ...
  const [groupByPs, setGroupByPs] = React.useState<boolean>(false);
  ...
  const [loanCurrency, setLoanCurrency] = React.useState<"LYD" | "USD">("LYD");
  const [loanMonthlyPayment, setLoanMonthlyPayment] = React.useState<string>("");
  ...
  const knownLeaveCodes = React.useMemo(() => {
  const set = new Set<string>();
  LEAVE_FULL_CODES.forEach(code => set.add(code));
  LEAVE_HALF_CODES.forEach(code => set.add(code));
  set.add("PL");
  set.add("PP");
  return set;
  }, []);

  const resolveVacationCode = React.useCallback((vac: VacationRecord): string => {
    const directRaw = (vac as any)?.code ?? vac.type;
    const normalize = (val: any) => String(val || '').toUpperCase();
    const direct = normalize(directRaw);
    if (direct && knownLeaveCodes.has(direct)) {
      return direct;
    }

    const idCan = (vac as any).id_can ?? (vac as any).typeCode ?? (vac as any).leaveCode;
    if (idCan != null) {
      const lt = leaveTypeMap[String(idCan)];
      if (lt?.code) {
        const ltCode = normalize(lt.code);
        if (knownLeaveCodes.has(ltCode)) {
          return ltCode;
        }
      }
    }

    const typeName = normalize((vac as any).type || (vac as any).typeName || (vac as any).comment);
    if (typeName.includes('SICK') || typeName.includes('مرض')) return 'SL';
    if (typeName.includes('EMERGENCY') || typeName.includes('طارئ')) return 'EL';
    if (typeName.includes('ANNUAL') || typeName.includes('annual')) return 'AL';
    if (typeName.includes('MATERNITY') || typeName.includes('أمومة')) return 'ML';
    if (typeName.includes('UNPAID') || typeName.includes('بدون')) return 'UL';
    if (typeName.includes('HALF')) return 'HL';
    if (typeName.includes('BEREAVEMENT') || typeName.includes('عزاء')) return 'BM';
    return 'AL';
  }, [knownLeaveCodes, leaveTypeMap]);
...
const computePPhPhf = (e: Payslip): PPhPhfVals => {
  ...
  // Debug for employee ID 26
  if (Number(e.id_emp) === 26) {
    console.log('[DEBUG] Employee 26 PHF data:', {
      id_emp: e.id_emp,
      agg: agg,
      vr: vr,
      phFullDays: agg?.phFullDays,
      phf_days: (vr as any).phf_days,
      presentWorkdays: e.presentWorkdays,
  ...
  // Debug for employee ID 26
  if (Number(e.id_emp) === 26) {
    console.log('[DEBUG] Employee 26 calculated values:', {
      pDays,
      phDays,
      phfDays,
      baseDailyLyd,
  ...
  if (!foodPerDay) {
    ...
    foodPerDay = W > 0 ? totalFoodLyd / W : 0;
  }

  const pLyd = pDays * baseDailyLyd;
  const pUsd = pDays * baseDailyUsd;

  // PH = double daily rate (no food) - ONLY if employee actually worked on holidays
  const phLyd = phDays * (baseDailyLyd * 2);
  const phUsd = phDays * (baseDailyUsd * 2);

  // PHF = double daily rate + food allowance - ONLY if employee actually worked on holidays
  const phfLyd = phfDays * (baseDailyLyd * 2 + foodPerDay);
  const phfUsd = phfDays * (baseDailyUsd * 2);

  // Debug logging for PH calculation
  if (phDays > 0 || phfDays > 0) {
    console.log(`[DEBUG] Employee ${e.id_emp} PH calculation:`, {
      phDays,
      phfDays,
      phLyd,
      phUsd,
      phfLyd,
      phfUsd,
      source: 'timesheet aggregation data'
    });
  }
  ...
  function computeFoodAllowance(emp: Payslip | null, v2: any) {
    ...
    const foodTotal = Number((v2 as any).wd_food_lyd ?? (v2 as any).food_allowance_lyd ?? 0);
    const workingDaysFood = Number((v2 as any).food_days ?? (v2 as any).workingDays ?? emp.workingDays ?? 0);
    // Try multiple sources for per-day food allowance
    let perDay = Number((emp as any).FOOD || (emp as any).FOOD_ALLOWANCE || (emp as any).food || (emp as any).food_allowance || 0);
    if (!perDay) perDay = Number((v2 as any).FOOD || (v2 as any).FOOD_ALLOWANCE || (v2 as any).food || (v2 as any).food_allowance || 0);
    if (!perDay && workingDaysFood > 0 && foodTotal > 0) perDay = foodTotal / workingDaysFood;
    ...
    // Fallback chain for paid days: v2.p_days -> tsAgg.presentP -> emp.presentWorkdays -> v2.present_days
    const paidDaysRaw = Number((v2 as any).p_days ?? agg?.presentP ?? emp.presentWorkdays ?? (v2 as any).present_days ?? 0) || 0;
    ...
    // If perDay is still 0 but we have foodTotal, calculate it from schedule
    if (!perDay && foodTotal > 0 && scheduleDays > 0) {
      perDay = foodTotal / scheduleDays;
    }
    ...
    if (hasFingerprint) {
      ...
      if (!fpDays && scheduleDays) {
        fpDays = Math.max(0, scheduleDays - Number(agg?.absenceDays ?? (v2 as any).absence_days ?? 0));
      }
      if (scheduleDays) fpDays = Math.min(fpDays, scheduleDays);
      
      // Also deduct food allowance for leave days for fingerprint employees
      // Employees on leave do not get food allowance paid out, regardless of fingerprint status
      fpDays = Math.max(0, fpDays - leaveUnits);
      
      // Debug logging for leave deduction
      if (leaveUnits > 0 && Number(emp.id_emp) === 26) {
        console.log(`[DEBUG] Employee ${emp.id_emp} food allowance deduction (fingerprint):`, {
          fpDaysBefore: fpDays + leaveUnits,
          leaveUnits,
          fpDaysAfter: fpDays,
          perDay,
          allowance: perDay * fpDays
        });
      }
      
      paidDays = Math.max(0, fpDays);
    } else {
      // For non-fingerprint employees, give food allowance for ALL working days except leave days
      // They don't need to punch in - just deduct leave days from schedule
      const scheduleDaysNonFp = workingDaysFood || Number(emp.workingDays ?? 0) || workingDaysExcludingFridays || 0;
      
      // Use foodEligibleNonFpDays if available (already excludes leave, Fridays, holidays)
      // Otherwise fall back to schedule days minus leave
      const foodEligibleDays = Number(agg?.foodEligibleNonFpDays ?? 0);
      let foodDays = foodEligibleDays > 0 ? foodEligibleDays : (scheduleDaysNonFp - leaveUnits);
      
      // Debug logging for non-FP food allowance
      console.log(`[DEBUG] Employee ${emp.id_emp} non-FP food allowance:`, {
        scheduleDaysNonFp,
        leaveUnits,
        foodEligibleNonFpDays: foodEligibleDays,
        foodDays,
        perDay
      });
      
      // Ensure we don't go negative
      paidDays = Math.max(0, foodDays);
    }
    const allowance = Number((perDay * paidDays).toFixed(2));
    
    // General debug logging for leave deduction
    if (leaveUnits > 0) {
      console.log(`[DEBUG] Employee ${emp.id_emp} food allowance with leave deduction:`, {
        hasFingerprint,
        leaveUnits,
        paidDays,
        perDay,
        allowance,
        totalWouldBeWithoutLeave: perDay * (paidDays + leaveUnits)
      });
    }
  ...
  function getPhAmounts(id_emp: number, opts?: { emp?: Payslip | null; v2?: any; workingDays?: number; baseLyd?: number; baseUsd?: number }) {
    try {
      ...
      // Use timesheet aggregation data instead of backend v2 data
      const agg = tsAgg?.[id_emp];
      const phDays = Number(agg?.phPartDays ?? 0) || 0;
      const phfDays = Number(agg?.phFullDays ?? 0) || 0;
      
      // Calculate daily rates
      const FIXED_MONTH_DAYS = 30;
      const baseLyd = opts?.baseLyd ?? (Number(emp?.baseSalary ?? 0) || 0);
      const baseUsd = opts?.baseUsd ?? (Number((emp as any)?.baseSalaryUsd ?? 0) || 0);
      const baseDailyLyd = baseLyd / FIXED_MONTH_DAYS;
      const baseDailyUsd = baseUsd / FIXED_MONTH_DAYS;
      
      // Calculate PH amounts from timesheet data
      const phLyd = phDays * (baseDailyLyd * 2);
      const phUsd = phDays * (baseDailyUsd * 2);
      const phfLyd = phfDays * (baseDailyLyd * 2);
      const phfUsd = phfDays * (baseDailyUsd * 2);
  ...
  function codeBadge(day?: any, schStartMin?: number | null, schEndMin?: number | null, empId?: number): string {
    ...
    const knownCodes = ['P','PM','A','PT','PL','PH','PHF','AL','SL','EL','ML','UL','HL','BM','XL','B1','B2','NI','NO','MO','IP','LI','EO','W','H','PP'];
    ...
    // Determine presence: either backend says present, OR there's entry/exit punch data
    const actualPresent = present || (entryMin != null && exitMin != null);
    
    if (!actualPresent) return 'A';
    
    // Check for PM (missing hours) BEFORE checking for late arrival or holiday
    const deltaMin = Number(day.deltaMin ?? 0);
    const missingFromWorked = expDur - worked;
    
    // PM detection: any of these conditions means missing hours
    // 1. deltaMin is negative and exceeds tolerance (backend-calculated)
    // 2. Worked hours are less than expected by more than tolerance
    // 3. miss variable (already calculated above) exceeds tolerance
    const hasMissingHours = 
      (deltaMin < 0 && Math.abs(deltaMin) > toleranceMinutes) ||
      (worked > 0 && expDur > 0 && missingFromWorked > toleranceMinutes) ||
      (miss > toleranceMinutes);
  ...
  const sortRows = React.useCallback((rows: Payslip[]) => {
    ...
    const getVal = (e: Payslip): any => {
      switch (sortKey) {
        case 'id': return Number(e.id_emp || 0);
  ...
  const startSession = () => {
    ...
    // Auto-tick the first employee
    if (displayedRows.length > 0) {
      markEmployeeProcessed(Number(displayedRows[0].id_emp));
    }
  };
  ...
  const markEmployeeProcessed = (empId: number) => {
    setProcessedEmployees(prev => new Set(Array.from(prev).concat(empId)));
  };

  const goToNextEmployee = () => {
    const employees = displayedRows;
    if (currentEmployeeIndex < employees.length - 1) {
      const nextIndex = currentEmployeeIndex + 1;
      setCurrentEmployeeIndex(nextIndex);
      // Auto-tick the checkbox for the next employee
      markEmployeeProcessed(Number(employees[nextIndex].id_emp));
    } else {
      // Last employee, mark session as completed
  ...
  const sessionTotals = React.useMemo(() => {
    if (!sessionMode || processedEmployees.size === 0) {
      return { totalAmountLyd: 0, totalAmountUsd: 0, employeeCount: 0 };
    }
    
    let totalLyd = 0;
    let totalUsd = 0;
    
    Array.from(processedEmployees).forEach(empId => {
      // Get salary data from v2Rows like PayrollPage does
      const vr = (v2Rows || []).find((x: any) => Number(x.id_emp) === empId) || {};
      const netLyd = Number((vr as any).net_salary_lyd || (vr as any).netLyd || (vr as any).D16 || 0) || 0;
      const netUsd = Number((vr as any).net_salary_usd || (vr as any).netUsd || (vr as any).C16 || 0) || 0;
    ...
    return {
      totalAmountLyd: totalLyd,
      totalAmountUsd: totalUsd,
      employeeCount: processedEmployees.size
    };
  }, [sessionMode, processedEmployees, v2Rows]);

  const currentEmployee = sessionMode ? displayedRows[currentEmployeeIndex] : null;

  // Keyboard navigation for session mode
  React.useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'ArrowDown' || event.key === 'ArrowRight') {
        ...
        event.preventDefault();
        if (currentEmployee) {
          markEmployeeProcessed(Number(currentEmployee.id_emp));
        }
  ...
  }, [sessionMode, currentEmployee, currentEmployeeIndex, displayedRows.length]);
...
const [commissionLoadingUI, setCommissionLoadingUI] = React.useState(false);
  ...
  const onRun = async () => {
    ...
    try {
      ...
      // Build per-employee timesheet aggregates using shared utility
      let vacationsByEmp: Record<number, VacationRecord[]> = {};
      try {
        const vacList = await getVacationsInRange(start, end);
        vacationsByEmp = (vacList || []).reduce((acc, vac) => {
          const id = Number(vac.id_emp);
          if (!Number.isFinite(id)) return acc;
          const state = String(vac.state || '').toLowerCase();
          if (state && !state.includes('approve') && !state.includes('accept')) return acc;
          if (!acc[id]) acc[id] = [];
          acc[id].push(vac);
          return acc;
        }, {} as Record<number, VacationRecord[]>);
      } catch {
        vacationsByEmp = {};
      }

      try {
        const agg: Record<number, PayrollAggData> = {};
        await Promise.all(
          employees.map(async (e) => {
            try {
              const days = await ensureTimesheetDays(e.id_emp);
              const empVacations = vacationsByEmp[Number(e.id_emp)] || [];

              // Build timesheet days array for shared calculation
              const timesheetDays: any[] = days.map((d: any, i: number) => {
                const vr = normalizedRows.find((x: any) => Number(x.id_emp) === Number(e.id_emp)) || {};
                ...
                const codeFromBadge = codeBadge(d, schStartMin ?? undefined, schEndMin ?? undefined, e.id_emp);
                const dateIso = `${year}-${String(month).padStart(2, "0")}-${String(i + 1).padStart(2, "0")}`;

                let code = codeFromBadge;
                if (!code) code = '';

                const matchingVacation = empVacations.find((vac) => {
                  const st = safeISO10((vac as any).date_depart);
                  const en = safeISO10((vac as any).date_end);
                  if (!st || !en) return false;
                  return dateIso >= st && dateIso <= en;
                });

                if (matchingVacation) {
                  const vacCode = resolveVacationCode(matchingVacation);
                  if (vacCode) {
                    const hasEntries = Boolean(d.present || d.entry || d.exit || (Array.isArray(d.punches) && d.punches.length));
                    if (!hasEntries || !code) {
                      code = vacCode;
                    }
                  }
                ...
                return {
                  ...
                  workMin: Number(d.workMin ?? 0) || 0,
                  present: typeof d.present === 'boolean' ? d.present : Boolean(code === 'P'),
                };
              });
  ...
  const buildPayslipPdf = async (emp: Payslip): Promise<{ dataUrl: string; blobUrl: string; filename: string }> => {
    ...
    const codeBadgePDF = (day: any, idx: number, schStartMin?: number | null, schEndMin?: number | null): string => {
      ...
      // BUT: If backend says 'P' but there are missing hours, we override to 'PM'
      if (rawCode) {
        ...
        const allowedDayCodes = ['P','PM','A','PT','PL','PH','PHF','NI','NO','MO','IP','LI','EO','W','PP'];
        if (allowedDayCodes.includes(rawCode)) {
          // Check if this is a 'P' code but has missing hours - should be PM instead
          if ((rawCode === 'P' || rawCode === 'PL' || rawCode === 'PT') && day) {
            // Calculate missing hours from T_START/T_END schedule vs entry/exit
            const parseTimeToMin = (s: any): number | null => {
              if (!s) return null;
              const txt = String(s);
              const m = txt.match(/(\d{1,2}):(\d{2})/);
              if (!m) return null;
            ...
            const schedStart = parseTimeToMin(day.T_START || day.t_start) ?? (schStartMin ?? 9*60);
            const schedEnd = parseTimeToMin(day.T_END || day.t_end) ?? (schEndMin ?? 17*60);
            const expectedDuration = Math.max(0, schedEnd - schedStart);
            
            const entryMin = parseTimeToMin(day.entry);
            const exitMin = parseTimeToMin(day.exit);
            const workedDuration = (entryMin != null && exitMin != null && exitMin > entryMin) 
              ? (exitMin - entryMin) : 0;
            
            // Calculate missing hours
            const missingMinutes = expectedDuration - workedDuration;
            
            // Also check deltaMin from backend
            const dm = Number(day.deltaMin ?? 0);
            const backendMissing = dm < 0 ? Math.abs(dm) : 0;
            
            // Use whichever shows more missing time
            const actualMissing = Math.max(missingMinutes, backendMissing);
            
            if (actualMissing > 30) {
              return 'PM'; // Override to PM when missing hours > 30 min
            }
          }
          return rawCode;
        }
      ...
      // Determine presence: either backend says present, OR there's entry/exit punch data
      const present = !!day.present || (entryMin != null && exitMin != null);
  ...
  try {
    ...
    const branchMap: Record<string, string> = { P1: 'Jraba Mall', P2: 'Jraba Main', P3: 'Ben Ashour', P4: 'Headquarters', P0: 'Headquarters' };
    const branchName = (() => {
    ...
    try {
      ...
      if (contractStartStr) {
        ...
        if (contractStart.isValid()) {
          ...
          contractStartLabel = contractStart.format('MM/DD/YYYY');
        }
      }
    } catch (e) {
  ...
  let earningsUsdTotal = 0;
  {
    ...
    // Gold commission: prefer computed value, fallback to backend v2
    const goldLyd = Math.max(0, goldBonusLYDComputed || Number((v2 as any).gold_bonus_lyd || 0));
    const goldUsd = Math.max(0, goldBonusUSDComputed || Number((v2 as any).gold_bonus_usd || 0));
    ...
    // Diamond commission: prefer computed value, fallback to backend v2
    const diaLyd = Math.max(0, diamondBonusLYDComputed || Number((v2 as any).diamond_bonus_lyd || 0));
    const diaUsd = Math.max(0, diamondBonusUSDComputed || Number((v2 as any).diamond_bonus_usd || 0));
  ...
  // Calculate absence deduction from actual absence days count × daily rate
  // Daily rate = base salary / 30 (standard month)
  const baseSalaryForAbs = Math.max(0, Number((v2 as any).base_salary_lyd || emp.baseSalary || 0));
  const baseSalaryUsdForAbs = Math.max(0, Number((v2 as any).base_salary_usd || 0));
  const dailyRateLyd = baseSalaryForAbs / 30;
  const dailyRateUsd = baseSalaryUsdForAbs / 30;
  // Use actual counted absence days (aCnt from the loop above), not backend value
  const absLyd = Math.max(0, Number((dailyRateLyd * aCnt).toFixed(2)));
  const absUsd = Math.max(0, Number((dailyRateUsd * aCnt).toFixed(2)));
  ...
  // Check if this is a USD loan by looking at the loan data
  const employeeLoans = loanRows?.filter((ln: any) => ln.currency === 'USD' && Number(ln.monthlyPayment || 0) > 0) || [];
  const hasUsdLoan = employeeLoans.length > 0;
  
  // For USD loans, the payment should be in USD column, for LYD loans in LYD column
  if (loanLyd > 0.0001 && !hasUsdLoan) {
    dRowsDual.push({ label: 'Loan Repayment', lyd: loanLyd, usd: 0 });
  } else if (loanUsd > 0.0001 && hasUsdLoan) {
    dRowsDual.push({ label: 'Loan Repayment', lyd: 0, usd: loanUsd });
  } else if (loanLyd > 0.0001 || loanUsd > 0.0001) {
    // Fallback: show in both if currency is unclear
    dRowsDual.push({ label: 'Loan Repayment', lyd: loanLyd, usd: loanUsd });
  }
    ...
    for (let idx = 0; idx < count; idx++) {
      ...
      const headerBg = (code: string): [number,number,number] | null => {
        switch (code) {
          ...
          case 'PM':  return [254,226,226]; // Present Missing - light red
      ...
      const isPmCode = badge === 'PM';
      const showCodes = ['P','PM','PH','PHF','PT','PL','A','H','AL','SL','EL','ML','UL','HL','BM','XL','B1','B2','NI','NO','MO','IP'].includes(badge) && !(isFri && !present);
      ...
      if (!(isFri && !present) && showCodes && badge) {
        ...
        if (badge === 'H') {
          ...
          doc.setTextColor(220, 38, 38); // Red for Present Missing
        } else if (isLeaveCode) {
      ...
      if (!isLeaveCode && badge !== 'H') {
        ...
        const schedStart = parseTimeToMin(day?.T_START || day?.t_start) ?? (schStartMinPDF ?? 9*60);
        const schedEnd = parseTimeToMin(day?.T_END || day?.t_end) ?? (schEndMinPDF ?? 17*60);
        const expectedDuration = Math.max(0, schedEnd - schedStart);
        ...
        // Calculate missing from schedule
        const missingFromSchedule = expectedDuration - workedDuration;
        ...
        // Use whichever shows more missing time
        const actualMissing = Math.max(missingFromSchedule, backendMissing);
        const showMissing = actualMissing > 30;
        
        if (showMissing) {
          const hh = Math.floor(actualMissing / 60);
          const mm = Math.floor(actualMissing % 60);
          const txt = `-${String(hh)}:${String(mm).padStart(2,'0')}`;
          
          doc.setFontSize(badge === 'PM' ? 6 : 5); // Slightly larger for PM
        ...
        const isPresentCode = ['P', 'PM', 'PL', 'PT', 'PH', 'PHF'].includes(badge);
        if (isPresentCode && day) {
          // Format entry/exit like TimeSheetsPage - handle various formats
          const formatEntryExit = (value?: string | null): string => {
            if (!value) return "";
            const raw = String(value);
            // If format is like "2026-01-15T08:30:00" or "2026-01-15 08:30:00", extract HH:MM
            if (raw.length >= 16) {
              return raw.slice(11, 16);
            }
            // If format is like "08:30:00", extract HH:MM
          ...
          const entryStr = formatEntryExit(day.entry);
          const exitStr = formatEntryExit(day.exit);
          doc.setFontSize(5);
          const textColor = badge === 'PM' ? [180, 38, 38] : [80, 80, 80];
          doc.setTextColor(textColor[0], textColor[1], textColor[2]);
          
          // Punch IN time - ABOVE the attendance code (top of cell)
          if (entryStr) {
            const entryW = doc.getTextWidth(entryStr);
            doc.text(entryStr, x + (wAdj - entryW) / 2, cellY + 7);
          }
          
          // Punch OUT time - BELOW the attendance code (bottom of cell)
          if (exitStr) {
            const exitW = doc.getTextWidth(exitStr);
            doc.text(exitStr, x + (wAdj - exitW) / 2, cellY + hCell - 2);
          }
  ...
  const counts: Record<string, number> = { 
    P:0, PM:0, A:0, PT:0, PHF:0, PH:0, PL:0,
  ...
  for (let d = 1; d <= dim; d++) {
    ...
    // Count all codes including 'A' from actual grid data
    if (badge && badge in counts && !skipFriday) counts[badge]++;
  }
  ...
  const allCodes = ['P','PM','A','PT','PHF','PH','PL','AL','SL','EL','ML','UL','HL','BM','XL','B1','B2','NI','NO','MO','IP','LI','EO'] as const;
  ...
  return (
                ...
                <>
                  ...
                  <FormControlLabel
                    control={
                      <Switch
                        checked={groupByPs}
                        onChange={(e) => setGroupByPs(e.target.checked)}
                        size="small"
                      />
                    }
                    label="Group by PS"
        ...
        {tab === "loans" && (
          <Box display="grid" gap={2}>
            ...
            {(() => {
              ...
              if (!emp) return null;
              const salary = Number(emp.baseSalary || 0);
              const salaryUsd = Number((emp as any).baseSalaryUsd || 0);
              const maxPrincipal = salary * loanMaxMultiple;
              const maxPrincipalUsd = salaryUsd * loanMaxMultiple;
              const hasUsdSalary = salaryUsd > 0;
              
              return (
                <Box display="grid" gap={1.5} maxWidth={400}>
                  <Typography variant="caption">
                    Salary: {salary.toFixed(2)} LYD{hasUsdSalary ? ` | ${salaryUsd.toFixed(2)} USD` : ""} — Max Principal: {(maxPrincipal || 0).toFixed(2)} LYD{hasUsdSalary ? ` | ${(maxPrincipalUsd || 0).toFixed(2)} USD` : ""} ({loanMaxMultiple}×)
                  </Typography>
                  {(() => {
                  ...
                  {hasUsdSalary && (
                    <TextField
                      ...
                      label="Currency"
                      value={loanCurrency}
                      onChange={(e) => setLoanCurrency(e.target.value as "LYD" | "USD")}
                      sx={{ maxWidth: 200 }}
                    >
                  ...
                  <TextField
                    ...
                    inputProps={{ step: "0.01", min: 0 }}
                    helperText={`Enter loan principal amount in ${loanCurrency}`}
                  />
                  <TextField
                    size="small"
                    type="number"
                    label={`Monthly Payment (${loanCurrency})`}
                    value={loanMonthlyPayment}
                    onChange={(e) => setLoanMonthlyPayment(e.target.value)}
                    inputProps={{ step: "0.01", min: 0 }}
                    helperText={`Custom monthly payment amount in ${loanCurrency} (optional)`}
                  />
                  <Button
                    variant="contained"
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const baseSalaryUsd = Number((emp as any).baseSalaryUsd || 0);
                        const maxPrincipal = loanCurrency === "USD" ? baseSalaryUsd * loanMaxMultiple : baseSalary * loanMaxMultiple;
                        const relevantSalary = loanCurrency === "USD" ? baseSalaryUsd : baseSalary;
                        
                        if (relevantSalary > 0 && principal > maxPrincipal) {
                          showUiError(`Loan exceeds maximum allowed (${loanMaxMultiple}× salary). Max: ${maxPrincipal.toFixed(2)} ${loanCurrency}`);
                          setAdjLoading(false);
                          return;
                        }
                        const vr =
                        ...
                        const customPayment = Number(loanMonthlyPayment) || 0;
                        const paymentParams = customPayment > 0 
                          ? { customMonthlyPayment: customPayment }
                        ...
                        try {
                          ...
                          if (toList) {
                            ...
                            doc2.text(`Principal: ${principal.toFixed(2)} ${loanCurrency}`, 36, 72);
                            doc2.text(`Start: ${String(year)}-${String(month).padStart(2, "0")}`, 36, 88);
                            const paymentText = customPayment > 0 
                              ? `Monthly Payment: ${customPayment.toFixed(2)} ${loanCurrency}`
                              : `Monthly Deduction: ${loanMonthlyPercent.toFixed(2)}%`;
                            doc2.text(paymentText, 36, 104);
                            ...
                            const html = `<div><p>Loan issued for employee <b>${empName}</b> (ID: ${adjEmpId}).</p><p>Principal: ${principal.toFixed(
                              2
                            )} ${loanCurrency}<br/>Start: ${String(year)}-${String(month).padStart(
                              2,
                              "0"
                            )}<br/>${paymentText}</p></div>`;
                            await sendPayslipClient({
                              to: toList,
                              subject,
                              html,
                    ...
                    <Box display="flex" alignItems="center" justifyContent="space-between">
                      <Box>
                        <Typography variant="body2">
                          ...
                          {Number(ln.remaining || 0).toFixed(2)} {ln.currency || "LYD"}
                        </Typography>
                        <Typography variant="caption" color="text.secondary">
                        ...
                        <Box>
                          {ln.history.map((h: any, i: number) => (
                            <Box key={i} display="flex" justifyContent="space-between">
                              ...
                              <Typography variant="caption">{Number(h.amount || 0).toFixed(2)} {ln.currency || "LYD"}</Typography>
                            </Box>
                          ))}
                        </Box>
                      ) : (
                  ...
                  {(() => {
                      ...
                      return rows.map((r: any, idx: number) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <Typography variant="body2" sx={{ fontWeight: 600, textAlign: 'right', color: '#65a8bf' }}>
                              {lydStr}
                            </Typography>
                            {showUsd && (
                              <Typography variant="body2" sx={{ fontWeight: 600, textAlign: 'right', color: '#b7a27d' }}>
                                {usdStr}
                              </Typography>
                            )}
                    ...
                    const TotalsRow = ({ totals }: { totals: { lyd: number; usd: number } }) => (
                      ...
                      >
                        ...
                        <Box />
                        <Typography variant="body2" sx={{ fontWeight: 700, textAlign: 'right', color: '#65a8bf' }}>
                          {totals.lyd ? totals.lyd.toFixed(2) : ''}
                        </Typography>
                        {showUsd ? (
                          <Typography variant="body2" sx={{ fontWeight: 700, textAlign: 'right', color: '#b7a27d' }}>
                        ...
                        >
                          <Box sx={{ px: 1, py: 0.75, bgcolor: 'background.default' }}>
                            ...
                            >
                              ...
                              <Typography variant="caption" sx={{ textAlign: 'right', color: '#65a8bf', fontWeight: 600 }}>LYD</Typography>
                              {showUsd && (
                                <Typography variant="caption" sx={{ textAlign: 'right', color: '#b7a27d', fontWeight: 600 }}>USD</Typography>
                              )}
    ...
    {!loading && result && (tab === "payroll" || isFullscreen) && (
      ...
      >
        ...
        >
          {/* Session Mode UI */}
          {sessionMode && currentEmployee && (
            <Card sx={{ mb: 1, border: "1px solid", borderColor: "divider" }}>
              <CardContent sx={{ p: 1.5 }}>
                <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
                  <Typography variant="subtitle2">
                    Employee {currentEmployeeIndex + 1} of {displayedRows.length} | Processed: {processedEmployees.size}
                  </Typography>
                  <Box display="flex" gap={1} alignItems="center">
                    <Typography variant="caption" color="text.secondary">
                      ← → navigate, Space tick, Esc exit
                    </Typography>
                    <Button variant="outlined" size="small" color="error" onClick={endSession}>
                      End
                    </Button>
                  </Box>
                </Box>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, py: 1, borderTop: '1px solid', borderColor: 'divider' }}>
                  <Checkbox
                    checked={processedEmployees.has(Number(currentEmployee.id_emp))}
                    onChange={() => markEmployeeProcessed(Number(currentEmployee.id_emp))}
                    size="small"
                  />
                  <Box sx={{ flex: 1 }}>
                    <Typography variant="body2" fontWeight="bold">
                      {currentEmployee.name} (ID: {currentEmployee.id_emp})
                    </Typography>
                    {(() => {
                      const breakdown = computePayBreakdown(currentEmployee.id_emp);
                      const vr = (v2Rows || []).find((x: any) => Number(x.id_emp) === Number(currentEmployee.id_emp)) || {};
                      const foodInfo = computeFoodAllowance(currentEmployee, vr);
                      const pVals = computePPhPhf(currentEmployee);
                      const netLyd = Math.max(0, Number((breakdown.earningsLyd - breakdown.deductionsLyd).toFixed(2)));
                      const netUsd = Math.max(0, Number((breakdown.earningsUsd - breakdown.deductionsUsd).toFixed(2)));
                      
                      return (
                        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, mt: 0.5 }}>
                          <Typography variant="caption">
                            <strong>Base:</strong> <span style={{ color: '#65a8bf' }}>{formatMoney(currentEmployee.baseSalary || 0)}</span>
                            {(currentEmployee as any).baseSalaryUsd ? <> | <span style={{ color: '#b7a27d' }}>{formatMoney((currentEmployee as any).baseSalaryUsd)}</span></> : null}
                          </Typography>
                          {pVals.pLyd > 0 && <Typography variant="caption"><strong>P:</strong> <span style={{ color: '#65a8bf' }}>{formatMoney(pVals.pLyd)}</span></Typography>}
                          {pVals.phLyd > 0 && <Typography variant="caption"><strong>PH:</strong> <span style={{ color: '#65a8bf' }}>{formatMoney(pVals.phLyd)}</span></Typography>}
                          {pVals.phfLyd > 0 && <Typography variant="caption"><strong>PHF:</strong> <span style={{ color: '#65a8bf' }}>{formatMoney(pVals.phfLyd)}</span></Typography>}
                          {foodInfo.allowance > 0 && <Typography variant="caption"><strong>Food:</strong> {formatMoney(foodInfo.allowance)} ({foodInfo.paidDays}d)</Typography>}
                          <Typography variant="caption">
                            <strong>Net:</strong> <span style={{ color: '#65a8bf' }}>{formatMoney(netLyd)}</span>
                            {netUsd > 0 ? <> | <span style={{ color: '#b7a27d' }}>{formatMoney(netUsd)}</span></> : null}
                          </Typography>
                        </Box>
                      );
                    })()}
                  ...
                  <Box display="flex" gap={0.5}>
                    <Button size="small" variant="outlined" onClick={goToPreviousEmployee} disabled={currentEmployeeIndex === 0}>←</Button>
                    <Button size="small" variant="contained" onClick={goToNextEmployee}>
                      {currentEmployeeIndex === displayedRows.length - 1 ? "✓" : "→"}
                    </Button>
                  </Box>
                </Box>
              </CardContent>
          ...
          {sessionCompleted && (
            <Card sx={{ border: "1px solid", borderColor: "divider", mb: 1 }}>
              <CardContent sx={{ p: 1.5 }}>
                <Typography variant="subtitle2" gutterBottom>
                  🎉 Session Completed!
                </Typography>
                <Box display="flex" gap={3}>
                  <Typography variant="caption">
                    <strong>{sessionTotals.employeeCount}</strong> employees
                  </Typography>
                  <Typography variant="caption">
                    <strong style={{ color: '#65a8bf' }}>{sessionTotals.totalAmountLyd.toFixed(2)}</strong> LYD
                  </Typography>
                  <Typography variant="caption">
                    <strong style={{ color: '#b7a27d' }}>{sessionTotals.totalAmountUsd.toFixed(2)}</strong> USD
                  </Typography>
                </Box>
              </CardContent>
            </Card>
          ...
          >
            ...
            >
              ...
              <TableRow>
                {/* ID column */}
                <TableCell
                  sortDirection={sortKey === "id" ? sortDir : (false as any)}
                ...
                <TableCell
                  ...
                  sx={{ width: 180, maxWidth: 180, position: "sticky", left: 50, zIndex: 6, bgcolor: "background.paper" }}
                >
                  <TableSortLabel
                    active={sortKey === "name"}
                      ...
                      >
                        <Box display="flex" flexDirection="column" alignItems="flex-end">
                          ...
                          <span style={{ fontSize: 9 }}>
                            <span style={{ color: "#65a8bf" }}>(LYD)</span> | <span style={{ color: "#b7a27d" }}>(USD)</span>
                          </span>
                        </Box>
                      </TableSortLabel>
                    </Box>
            ...
            <TableBody>
              {(() => {
                ...
                const rowsToRender: Array<{ type: 'header' | 'employee'; psKey?: string; employee?: Payslip; count?: number }> = [];
                
                if (groupByPs) {
                  // Group employees by PS
                  const grouped: Record<string, Payslip[]> = {};
                  displayedRows.forEach((emp: Payslip) => {
                    const psVal = formatPs(emp.PS) || 'Unassigned';
                    if (!grouped[psVal]) grouped[psVal] = [];
                    grouped[psVal].push(emp);
                  });
                  
                  // Sort PS keys
                  const psKeys = Object.keys(grouped).sort((a, b) => {
                  ...
                  psKeys.forEach((psKey) => {
                    rowsToRender.push({ type: 'header', psKey, count: grouped[psKey].length });
                    grouped[psKey].forEach((emp) => {
                      rowsToRender.push({ type: 'employee', employee: emp });
                    });
                  });
                } else {
                  // No grouping - just add all employees
                  displayedRows.forEach((emp: Payslip) => {
                    rowsToRender.push({ type: 'employee', employee: emp });
                  });
                }
                ...
                return rowsToRender.map((item, idx) => {
                  if (item.type === 'header') {
                    return (
                      <TableRow key={`ps-header-${item.psKey}`} sx={{ bgcolor: 'primary.light' }}>
                        <TableCell colSpan={100} sx={{ py: 1, fontWeight: 700, color: 'primary.contrastText' }}>
                          {item.psKey} ({item.count} employees)
                        </TableCell>
                      </TableRow>
                    );
                  }
                  
                  // Employee row - use the exact same JSX as before
                  ...
                  const isCurrentEmployee = sessionMode && currentEmployee && Number(e.id_emp) === Number(currentEmployee.id_emp);
                  const isProcessed = processedEmployees.has(Number(e.id_emp));
                  ...
                  return (
                    <TableRow 
                      ...
                      sx={{ 
                        ...
                        ...(sessionMode && {
                          opacity: isCurrentEmployee ? 1 : 0.4,
                          bgcolor: isCurrentEmployee ? "action.selected" : "inherit",
                    ...
                    >
                      <Box display="flex" alignItems="center" gap={1.25}>
                        {sessionMode && (
                          <Checkbox
                            checked={isProcessed}
                            onChange={() => markEmployeeProcessed(Number(e.id_emp))}
                            color="success"
                            size="small"
                            sx={{ ml: 1 }}
                          />
                        )}
                        {requiresFingerprint(e.id_emp) ? (
                          <FingerprintIcon sx={{ width: 28, height: 28, color: "text.secondary" }} />
                        ) : (
                          <Avatar
                        ...
                        <Box display="flex" flexDirection="column" sx={{ maxWidth: 220, overflow: "hidden", textOverflow: "ellipsis" }}>
                          <Typography fontWeight={500} noWrap sx={{ cursor: "pointer" }} onClick={() => openCalendar(e.id_emp, nameMap[e.id_emp] ?? e.name)}>
                            {nameMap[e.id_emp] ?? e.name}
                          </Typography>
                          <Typography variant="caption">
                            {(() => {
                              const vr = (v2Rows || []).find((x: any) => Number(x.id_emp) === Number(e.id_emp)) || {};
                              const pick = (s: any) => { if (!s) return ""; const m = String(s).match(/(\d{1,2}):(\d{2})/); return m ? m[0] : ""; };
                              ...
                              const psInfo = e.PS != null ? `${formatPs(e.PS)}` : "";
                              const designationInfo = e.designation ? `• ${e.designation}` : "";
                              return `${psInfo} ${designationInfo}${sch}`.trim();
                            })()}
                          </Typography>
                        </Box>
                      </Box>
                    ...
                    {cols.baseSalary && (
                      <TableCell align="right" sx={{ width: 110 }}>
                        {(() => {
                          ...
                          return (
                            <Box component="span">
                              <Box component="span" sx={{ color: "#65a8bf" }}>{formatMoney(lyd)}</Box>
                              {usd ? <>{" | "}<Box component="span" sx={{ color: "#b7a27d" }}>{formatMoney(usd)}</Box></> : null}
                            </Box>
                          );
                        })()}
                      </TableCell>
                    ...
                    {cols.p && (
                      <TableCell align="right" sx={{ width: 110 }}>
                        {(() => {
                          ...
                          const tooltipText = `P: ${vals.pDays || 0} days`;
                          return (
                            <Tooltip title={tooltipText} arrow>
                              <Box component="span" sx={{ cursor: 'help' }}>
                                <Box component="span" sx={{ color: "#65a8bf" }}>{formatMoney(vals.pLyd)}</Box>
                                {vals.pUsd ? <>{" | "}<Box component="span" sx={{ color: "#b7a27d" }}>{formatMoney(vals.pUsd)}</Box></> : null}
                              </Box>
                            </Tooltip>
                          );
                        })()}
                    ...
                    {cols.ph && (
                      <TableCell align="right" sx={{ width: 110 }}>
                        {(() => {
                          ...
                          const tooltipText = `PH: ${vals.phDays || 0} days`;
                          return (
                            <Tooltip title={tooltipText} arrow>
                              <Box component="span" sx={{ cursor: 'help' }}>
                                <Box component="span" sx={{ color: "#65a8bf" }}>{formatMoney(vals.phLyd)}</Box>
                                {vals.phUsd ? <>{" | "}<Box component="span" sx={{ color: "#b7a27d" }}>{formatMoney(vals.phUsd)}</Box></> : null}
                              </Box>
                            </Tooltip>
                          );
                        })()}
                    ...
                    {cols.phf && (
                      <TableCell align="right" sx={{ width: 110 }}>
                        {(() => {
                          ...
                          const tooltipText = `PHF: ${vals.phfDays || 0} days`;
                          return (
                            <Tooltip title={tooltipText} arrow>
                              <Box component="span" sx={{ cursor: 'help' }}>
                                <Box component="span" sx={{ color: "#65a8bf" }}>{formatMoney(vals.phfLyd)}</Box>
                                {vals.phfUsd ? <>{" | "}<Box component="span" sx={{ color: "#b7a27d" }}>{formatMoney(vals.phfUsd)}</Box></> : null}
                              </Box>
                            </Tooltip>
                          );
                        })()}
                    ...
                    {cols.food && (
                      <TableCell align="right" sx={{ width: 72 }}>
                        {(() => {
                          ...
                          const tooltipText = `${foodInfo.paidDays} days × ${formatMoney(foodInfo.perDay)}/day`;
                          return (
                            <Tooltip title={tooltipText} arrow>
                              <span style={{ cursor: 'help' }}>
                                {Number.isFinite(foodInfo.allowance) ? formatMoney(foodInfo.allowance) : "0.00"}
                              </span>
                            </Tooltip>
                          );
                        })()}
                    ...
                    {cols.diamond && (
                      <TableCell align="right" sx={{ width: 120 }}>
                        <Box component="span">
                          {diamondGrams > 0 ? <><Box component="span" sx={{ fontWeight: 700 }}>{diamondGrams.toLocaleString(undefined, { maximumFractionDigits: 2 })} g</Box>{"  "}</> : null}
                          <Box component="span" sx={{ color: "#65a8bf", fontWeight: 700 }}>{formatMoney(diamondLyd)}</Box>
                        </Box>
                      </TableCell>
                    )}
                    ...
                    {cols.loans && (
                      <TableCell align="right" sx={{ width: 78 }}>
                        {(() => {
                          ...
                          return (
                            <Box display="flex" flexDirection="column" alignItems="flex-end">
                              {thisMonthNeg ? <Box component="span" sx={{ color: "error.main" }}>{formatMoney(thisMonthNeg)}</Box> : null}
                    ...
                    {cols.latency && (
                      <TableCell align="right" sx={{ width: 90 }}>
                        {(() => {
                          ...
                          const timeStr = latencyMin > 0 ? `${hh}h${mm}m` : '';
                          return (
                            <Tooltip title={timeStr ? `Missing: ${timeStr}` : ''} arrow>
                              <Box component="span">
                                <Box component="span" sx={{ color: "#65a8bf" }}>{formatMoney(latencyLyd)}</Box>
                                {latencyUsd ? <>{" | "}<Box component="span" sx={{ color: "#b7a27d" }}>{formatMoney(latencyUsd)}</Box></> : null}
                              </Box>
                            </Tooltip>
                          );
                        })()}
                    ...
                    <TableCell align="right" sx={{ width: 90 }}>
                      {(() => {
                        ...
                        return <Box component="span" sx={{ color }}>{formatMoney(Math.abs(netAdj))}</Box>;
                      })()}
                    </TableCell>
                    ...
                    {cols.absence && (
                      <TableCell align="right" sx={{ width: 110 }}>
                        {(() => {
                          ...
                          // Use tsAgg absenceDays (counted from actual 'A' codes) instead of backend value
                          const absenceDays = Number(tsAgg?.[Number(e.id_emp)]?.absenceDays || 0) || 0;
                          if (!absenceLyd && !absenceUsd) return "";
                          return (
                            <Tooltip title={absenceDays ? `${absenceDays} days` : ''} arrow>
                              <Box component="span">
                                <Box component="span" sx={{ color: "#65a8bf" }}>{formatMoney(absenceLyd)}</Box>
                                {absenceUsd ? <>{" | "}<Box component="span" sx={{ color: "#b7a27d" }}>{formatMoney(absenceUsd)}</Box></> : null}
                              </Box>
                            </Tooltip>
                          );
                        })()}
                    ...
                    <TableCell align="right" sx={{ width: 120 }}>
                      {breakdown.grossLyd || breakdown.grossUsd ? (
                        <Box component="span">
                          <Box component="span" sx={{ color: "#65a8bf" }}>{formatMoney(breakdown.grossLyd)}</Box>
                          {breakdown.grossUsd ? <>{" | "}<Box component="span" sx={{ color: "#b7a27d" }}>{formatMoney(breakdown.grossUsd)}</Box></> : null}
                        </Box>
                      ) : ""}
                    </TableCell>
                    ...
                    <TableCell align="right" sx={{ width: 120 }}>
                      {netLydVal || netUsdVal ? (
                        <Box component="span">
                          <Box component="span" sx={{ color: "#65a8bf" }}>{formatMoney(netLydVal)}</Box>
                          {netUsdVal ? <>{" | "}<Box component="span" sx={{ color: "#b7a27d" }}>{formatMoney(netUsdVal)}</Box></> : null}
                        </Box>
                      ) : ""}
                    </TableCell>

                    <TableCell align="right" sx={{ width: 60, position: "sticky", right: 0, zIndex: 2, bgcolor: "background.paper" }}>
                      <Box display="flex" flexDirection="row" gap={0.5} alignItems="center" justifyContent="flex-end">
                        ...
                        <IconButton size="small" color="success" onClick={() => exportPdfClient(e)}>
              ...
              {/* Totals row */}
                ...
                >
                  ...
                  <TableCell />
                  <TableCell sx={{ position: "sticky", left: 50, bottom: 0, zIndex: 7, bgcolor: "background.paper" }}>
                  ...
                  {cols.food && (
                    <TableCell align="right">
                      <strong>{formatMoney(totals.food || 0)}</strong>
                    </TableCell>
                  )}
                  ...
                  {cols.fuel && (
                    <TableCell align="right">
                      <strong>{formatMoney(totals.fuel || 0)}</strong>
                    </TableCell>
                  )}
                  ...
                  {cols.comm && (
                    <TableCell align="right">
                      <strong>{formatMoney(totals.comm || 0)}</strong>
                    </TableCell>
                  )}
                  ...
                  {cols.latency && (
                    <TableCell align="right">
                      <strong>
                        <Box component="span" sx={{ color: "#65a8bf" }}>
                          {formatMoney(totals.latencyLyd || 0)}
                        </Box>
                        {totals.latencyUsd ? (
                          <>
                            {" | "}
                  ...
                  {/* Adjustments */}
                  <TableCell align="right">
                    <strong>{formatMoney(totals.adj || 0)}</strong>
                  </TableCell>
                  ...
                  {cols.absence && (
                    <TableCell align="right">
                      <strong>
                        <Box component="span" sx={{ color: "#65a8bf" }}>
                          {formatMoney(totals.absenceLyd || 0)}
                        </Box>
                        {totals.absenceUsd ? (
                          <>
                            {" | "}
                  ...
                  {/* Actions */}
                  <TableCell align="right" sx={{ position: "sticky", right: 0, bottom: 0, zIndex: 7, bgcolor: "background.paper" }} />
                </TableRow>
              </TableBody>
            </Table>
          </TableContainer>
                      ...
                      <Box>
                        <Box sx={{ mb: 2, p: 1.5, bgcolor: "background.paper", border: "1px solid", borderColor: "divider", borderRadius: 1 }}>
                          ...
                          <Box display="flex" flexWrap="wrap" gap={1.5}>
                            ...
                            <Typography variant="caption" sx={{ color: '#dc2626' }}>PM: Present but Missing Hours</Typography>
