...
const CashBookReports = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get<Accounts[]>(`${apiUrlAccounts}/all`, {
  ...
  const transactionsWithBalance = React.useMemo(() => {
    ...
    const rows = transactions.map((row) => {
      ...
      return {
        ...
        runningCurrBalance: currBalance,
      };
    });
    ...
    const openingRow = {
      ...
      coa: {
        Acc_No: selectedAccount,
        Name_M:
          accounts.find((acc) => acc.Acc_No === selectedAccount)?.Name_M || "",
      },
  ...
  type TableRow = Transaction & {
    runningBalance: number;
    runningCurrBalance: number;
  };
  const columns: MRT_ColumnDef<TableRow>[] = [
    ...
    },
    {
      ...
      header: "Balance",
      Cell: ({ row }) =>
        Number(row.original.runningBalance || 0).toLocaleString(undefined, {
          minimumFractionDigits: 3,
          maximumFractionDigits: 3,
        }),
    },
    ...
    },
    {
      ...
      size: 120,
      Cell: ({ row }) =>
        Number(row.original.runningCurrBalance || 0).toLocaleString(undefined, {
          minimumFractionDigits: 3,
          maximumFractionDigits: 3,
        }),
    },
    ...
    <Box>
      <Box sx={{ display: "flex", gap: 2, mb: 2 }}>
        ...
        <Autocomplete
          ...
          onChange={(_, newValue) =>
            setSelectedAccount(newValue ? newValue.Acc_No : "")
          }
          sx={{ minWidth: 400 }}
          size="small"
          renderInput={(params) => (
      ...
      >
        <div>
          ...
          &nbsp;
          <span style={{ color: "#388e3c", fontSize: 16, fontStyle: "italic" }}>
            {accounts.find((acc) => acc.Acc_No === selectedAccount)?.Name_M ||
              ""}
          </span>
          &nbsp;LYD Balance:{" "}
          {(() => {
            if (transactionsWithBalance.length > 0) {
              ...
              return Number(last.runningBalance || 0).toLocaleString(
                undefined,
                { minimumFractionDigits: 3, maximumFractionDigits: 3 }
              );
            }
            return Number(openingBalance || 0).toLocaleString(undefined, {
              minimumFractionDigits: 3,
              maximumFractionDigits: 3,
            });
          })()}
        ...
        <div>
          ...
          {(() => {
            if (transactionsWithBalance.length > 0) {
              const last =
                transactionsWithBalance[transactionsWithBalance.length - 1];
              return Number(last.runningCurrBalance || 0).toLocaleString(
                undefined,
                { minimumFractionDigits: 3, maximumFractionDigits: 3 }
              );
            }
