...
const Expenses = (props: Props) => {
  ...
  // Removed refreshFlag: we'll reapply active filters explicitly after changes
  ...
  const fetchData = useCallback(
    async (fromDate?: string, toDate?: string) => {
      ...
      try {
        ...
        // If from/to provided filter client-side, else leave table empty by default
        if (fromDate && toDate) {
          const from = new Date(fromDate);
          const to = new Date(toDate);
          to.setHours(23, 59, 59, 999);
          const filtered = res.data.filter((r) => {
            const dStr = r.date_trandsaction;
  ...
  // On mount: load accounts, customers and employees only. Do not fetch expense rows until a filter is applied.
  useEffect(() => {
    fetchDataAccounts();
    fetchCustomers();
    fetchEmployee();
  }, [fetchDataAccounts, fetchCustomers, fetchEmployee]);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      // reapply the active filter (if any)
      applyCurrentFilter();
    } catch (error) {
  ...
  const handleCloseInvoiceForm = (success: boolean = false) => {
    ...
    if (success) {
      // reapply active filter after create/update
      applyCurrentFilter();
    }
  };
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setEditExpense((prev) => {
      ...
      if (name === "montant" || name === "rate") {
        const montant =
          name === "montant" ? Number(value) : Number(updated.montant);
  ...
  const handleSubmit = async () => {
    ...
    if (editExpense.montant === undefined || editExpense.montant === null)
      validationErrors.montant = "Amount is required";
    if (!editExpense.Account_number1)
      validationErrors.Account_number1 = "Debit account is required";
    if (!editExpense.Account_number2)
      validationErrors.Account_number2 = "Credit account is required";
    // Ensure date_trandsaction is always a valid date string
    if (
      !editExpense.date_trandsaction ||
      isNaN(Date.parse(editExpense.date_trandsaction))
    ) {
    ...
    () => [
      {
        ...
        Cell: ({ row }) => {
          ...
          return (
            ...
            >
              <span>
                <strong>Txn No:</strong> {txn}
              </span>
              <span>
                <strong>Date:</strong> {date}
              </span>
              <span>
                <strong>Document:</strong> {docNum}
              </span>
            </Box>
          );
        },
      ...
      },
      {
        ...
        Cell: ({ cell, row }) => {
          const customer = customers.find(
            (c) => c.id_client === row.original.client
          );
...

      {
        ...
        Cell: ({ row }) => {
          ...
          const lyd = (
            row.original.montant_net ?? row.original.montant * rate
          ).toLocaleString("en");
          return (
            <Box
              sx={{
      ...
      },
      {
        ...
        Cell: ({ cell }) => {
          const emp = employees.find(
            (e) => Number(e.ID_EMP) === Number(cell.getValue())
          );
          return (
            <span>
              {emp ? `${emp.NAME} (${emp.ID_EMP})` : String(cell.getValue())}
            </span>
          );
        },
      },
  ...
  return (
    <Box p={2}>
      ...
      >
        <DialogTitle>
          {isEditMode
            ? `Edit Expense #${editExpense.ID_transaction}`
            : "Create New Expense"}
        </DialogTitle>
        <DialogContent>
          <Box sx={{ mt: 2, display: "flex", flexDirection: "column", gap: 2 }}>
            ...
            <Autocomplete
              ...
              value={
                customers.find((c) => c.id_client === editExpense.client) ||
                null
              }
            ...
            <Box sx={{ display: "flex", gap: 2 }}>
              <Autocomplete
                ...
                value={
                  dataAccounts.find(
                    (c) => c.Acc_No === editExpense.Account_number1
                  ) || null
                }
                onChange={(event, newValue) => {
                  setEditExpense((prev) => ({
              ...
              <Autocomplete
                ...
                value={
                  dataAccounts.find(
                    (c) => c.Acc_No === editExpense.Account_number2
                  ) || null
                }
                onChange={(event, newValue) => {
                  setEditExpense((prev) => ({
            ...
            <Box sx={{ display: "flex", gap: 2 }}>
              ...
              <TextField
                ...
                type="number"
                value={
                  editExpense.montant_net ??
                  editExpense.montant * (editExpense.rate || 1)
                }
                fullWidth
              />
            </Box>
            ...
            <Autocomplete
              ...
              value={
                employees.find(
                  (e) => Number(e.ID_EMP) === Number(editExpense.ref_emp)
                ) || null
              }
              onChange={(event, newValue) => {
                setEditExpense((prev) => ({
      ...
      <Box sx={{ display: "flex", gap: 2, alignItems: "center", mb: 2 }}>
        <FormControl sx={{ minWidth: 220 }}>
          <InputLabel id="expenses-filter-label">Filter Range</InputLabel>
          <Select
            labelId="expenses-filter-label"
            value={selectedRange}
            label="Filter Range"
