...
const CustomersReports = ({ type: initialType }: { type?: 'gold' | 'diamond' | 'watch' }) => {
    const [type, setType] = useState<'all' | 'gold' | 'diamond' | 'watch'>(initialType || 'all');
    const [currency, setCurrency] = useState<'all' | 'LYD' | 'USD' | 'EUR'>('all');
    ...
    const fetchImageBlobs = async (picint: number, urls: string[]) => {
        ...
        for (const url of urls) {
            try {
                ...
                console.log('[fetchImageBlobs] Fetching blob for picint', picint, 'url:', imgUrl); // Debug log
                ...
                console.error('[fetchImageBlobs] Error fetching blob for picint', picint, 'url:', url, err); // Debug log
            }
        }
    ...
    useEffect(() => {
        ...
        axios.get(`http://102.213.182.8:9000/invoices/allDetailsPC`, {
            ...
            params: {
                ...
                ...(selectedCustomer && selectedCustomer.id_client ? { client: selectedCustomer.id_client } : {})
            }
        })
            .then(res => {
                // Detect if response is HTML (not JSON)
                if (typeof res.data === 'string' && res.data.trim().startsWith('<!DOCTYPE html')) {
                    setErrorMsg('Server returned an HTML page instead of data. This may indicate a backend error, downtime, or invalid API endpoint.');
                    setData([]);
                    return;
                }
            ...
            .catch((err) => {
                setErrorMsg('Failed to fetch data from server. Please check your connection or contact support.');
                setData([]);
            })
            .finally(() => setLoading(false));
    }, [type, periodFrom, periodTo, ps, isChira, isWholeSale, selectedCustomer, currency]);
    ...
    const invoiceRows = filterInvoicesByCurrency(
        safeData.filter(row => Array.isArray(row.ACHATs) && row.ACHATs.length > 0 && row.num_fact),
        currency
    );
    // Revenue rows: must NOT have ACHATs and must NOT have num_fact
    const revenueRows = safeData.filter(row => (!row.ACHATs || row.ACHATs.length === 0) && !row.num_fact);
    ...
    async function exportTableToHtml() {
        ...
        rowsWithBalance.forEach((row: any) => {
            // Type, Date, Ref, POS
            let type = row.statementType === 'debit' ? 'Invoice' : row.statementType === 'credit' ? 'Revenue' : row.type || '';
            ...
            if (row.id === 'opening-balance') {
                ...
                if (remise > 0) {
                    ...
                    savedTotal = Number(totalRemiseFinal) - (Number(totalRemiseFinal) * Number(remise_per) / 100);
                }
    ...
    const allStatementRows = [
        ...
        ...revenueData.map(rev => {
            ...
            let credit_lyd = null, credit_usd = null, credit_eur = null;
            if (rev.currency === 'LYD') {
    ...
    const openingRows = allStatementRows.filter(r => new Date(r.date) < new Date(periodFrom));
    ...
    // Main period rows
    const mainRows = allStatementRows.filter(r => new Date(r.date) >= new Date(periodFrom) && new Date(r.date) <= new Date(periodTo));
    // Running balances
    ...
    const rowsWithBalance = [
        { id: 'opening-balance', type: 'Opening Balance', date: periodFrom, description: 'Opening Balance', debit: '', credit: '', balance: openingBalance, balance_lyd: openingBalance, balance_usd: openingBalanceUsd, balance_eur: openingBalanceEur },
        ...mainRows.map((row, idx) => {
            ...
            return { ...row, balance: runningBalanceLyd, balance_lyd: runningBalanceLyd, balance_usd: runningBalanceUsd, balance_eur: runningBalanceEur };
        })
    ];
    ...
    const statementColumns: MRT_ColumnDef<any>[] = [
        {
            ...
            Cell: ({ row }) => {
                ...
                const type = row.original.statementType === 'debit' ? 'Invoice' : row.original.statementType === 'credit' ? 'Revenue' : row.original.type || '';
                const date = row.original.date;
                const ref = row.original.num_fact || row.original.id_acc_cli || '';
                const psVal = row.original.ps || '';
                return (
                    <Box sx={{ whiteSpace: 'pre-line', display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                        ...
                        {!isOpening && (
                            <span style={{ fontWeight: 500 }}><span style={{ color: '#888', fontSize: 12 }}>Ref:</span> {ref}</span>
                        )}
                        {!isOpening && (
                            <span style={{ fontWeight: 500 }}><span style={{ color: '#888', fontSize: 12 }}>Point of Sale:</span> {psVal}</span>
                        )}
                    </Box>
                );
            },
...

        {
            ...
            Cell: ({ row }) => {
                ...
                if (!debit) return (
                    ...
                    <>
                        <Box sx={{ whiteSpace: 'pre-line', display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                            <div><b>Account:</b> {'[AR]'}</div>
                            <div><b>Account Name:</b> {'Account Receivable'}</div>
                        </Box>
                    </>
                )
                ...
                return (
                    <Box sx={{ whiteSpace: 'pre-line', display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                        <div><b>Account:</b> {debit.Acc_No || ''}</div>
                        <div><b>Account Name:</b> {debit.Name_M || ''}</div>
                    </Box>
                );
            },
        },
        {
            ...
            Cell: ({ row }) => {
                ...
                if (!credit) return (
                    ...
                    <>
                        <Box sx={{ whiteSpace: 'pre-line', display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                            <div><b>Account:</b> {'[SR]'}</div>
                            <div><b>Account Name:</b> {'Sales Revenue'}</div>
                        </Box>
                    </>
                );
                return (
                    <Box sx={{ whiteSpace: 'pre-line', display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                        <div><b>Account:</b> {credit.Acc_No || ''}</div>
                        <div><b>Account Name:</b> {credit.Name_M || ''}</div>
                    </Box>
                );
            },
        },
                        ...
                        <div style={{ whiteSpace: 'pre-line', fontSize: 12 }}>
                            ...
                            {remise > 0 && (
                                <div>
                                    ...
                                    <span style={{ marginLeft: 6, color: '#d32f2f' }}>{formatNumber(remise)}</span>
                                </div>
                            )}
                            {/* Discount Percentage */}
                            {remise_per > 0 && (
                                <div>
                                    ...
                                    <span style={{ marginLeft: 6, color: '#d32f2f' }}>{formatNumber(remise_per)}</span>
                                </div>
                            )}
                            ...
                            {(remise > 0 || remise_per > 0) && (
                                <div>
                                    <span style={{ fontWeight: 'bold', color: '#388e3c' }}>Saved Total:</span>
                                    <span style={{ marginLeft: 6 }}>
                                        {row.original?.ACHATs?.[0]?.Fournisseur?.TYPE_SUPPLIER?.toLowerCase().includes('gold')
                ...
                if (row.original.statementType === 'credit') {
                    ...
                    return (
                        <div style={{ whiteSpace: 'pre-line', fontSize: 12 }}>
                            {montantCurrency !== 0 && (
                                <div>
                                    <span style={{ fontWeight: 'bold', color: '#1976d2' }}>Amount:</span>
                                    <span style={{ marginLeft: 6 }}>{formatNumber(montantCurrency)} {row.original.currency}</span>
                                </div>
                            )}
                            {montant !== 0 && (
                                <div>
                                    <span style={{ fontWeight: 'bold', color: '#1976d2' }}>Amount (LYD):</span>
                                    <span style={{ marginLeft: 6 }}>{formatNumber(montant)} LYD</span>
                                </div>
                            )}
                        </div>
                    );
...

        {
            ...
            size: 250,
            Cell: ({ row }) => {
                if (row.original.id === 'opening-balance') return <span style={{ fontWeight: 600 }}>Opening Balance</span>;
                if (row.original.statementType === 'debit') {
                            ...
                            {row.original._productDetails?.length === 0 ? (
                                ...
                                row.original._productDetails.map((d: any, idx: number) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <span style={isGift ? { color: 'orangered', fontWeight: 500 } : {}}>
                                                {idart}
                                            </span>
                                            {isGift && (
                                                <span
                                                ...
                                                >
                                                    ...
                                                    <span style={{ marginLeft: 1, fontWeight: 500, fontSize: 13 }}> Gift</span>
                                                </span>
                                            )}
                                        </div>
                                    );
                    ...
                    return (
                        <Box sx={{ whiteSpace: 'pre-line', display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                            <div><b>Revenue ID:</b> {row.original.id_acc_cli}</div>
                            <div><b>Comment:</b> {row.original.comment}</div>
                        </Box>
                    );
                }
            },
                ...
                if (row.original.statementType === 'debit') {
                    ...
                    return (
                        <Box>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'row', gap: 16, marginTop: 4, flexWrap: 'wrap' }}>
                                ...
                                {row.original._productDetails?.length === 0 ? (
                                    ...
                                    row.original._productDetails.map((d: any, idx: number) => {
                                        ...
                                        const urls = picint && imageBlobUrls[picint] ? imageBlobUrls[picint]
                                            : (imageUrls[picint] ? imageUrls[picint].map(url => `${API_BASEImage}/${url}`) : []);

                                        return (
                                            <Box  >

                                                <Box sx={{ display: 'inline-flex', flexDirection: 'row', gap: 2, mt: 0.5 }}>
                                                    {urls.length > 0 ? (
                                                        ...
                                                        <span style={{ color: '#aaa', fontSize: 10 }}>No Image</span>
                                                    )}
                                                </Box>
                                            </Box>
                                        );
        ...
        <Box >
            <React.Fragment>
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, gap: 2, flexWrap: 'wrap' }}>
                    <Typography variant="h5" sx={{ mr: 2 }}>{MODEL_LABELS[type]} - Customer Reports</Typography>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 2, gap: 4, flexWrap: 'wrap' }}>
                    {(currency === 'all' || currency === 'LYD') && (
                        <Typography variant="subtitle1" sx={{ fontWeight: 'bold', color: rowsWithBalance.length > 0 && rowsWithBalance[rowsWithBalance.length - 1].balance_lyd > 0 ? 'red' : 'inherit' }}>
                            Final Balance (LYD): {rowsWithBalance.length > 0 ? formatNumber(rowsWithBalance[rowsWithBalance.length - 1].balance_lyd) : '0.00'} LYD
                        </Typography>
                    )}
                    {(currency === 'all' || currency === 'USD') && (
                        <Typography variant="subtitle1" sx={{ fontWeight: 'bold', color: rowsWithBalance.length > 0 && rowsWithBalance[rowsWithBalance.length - 1].balance_usd > 0 ? 'red' : 'inherit' }}>
                            Final Balance (USD): {rowsWithBalance.length > 0 ? formatNumber(rowsWithBalance[rowsWithBalance.length - 1].balance_usd) : '0.00'} USD
                        </Typography>
                    )}
                    {(currency === 'all' || currency === 'EUR') && (
                        <Typography variant="subtitle1" sx={{ fontWeight: 'bold', color: rowsWithBalance.length > 0 && rowsWithBalance[rowsWithBalance.length - 1].balance_eur > 0 ? 'red' : 'inherit' }}>
                            Final Balance (EUR): {rowsWithBalance.length > 0 ? formatNumber(rowsWithBalance[rowsWithBalance.length - 1].balance_eur) : '0.00'} EUR
                        </Typography>
                    )}
                </Box>
