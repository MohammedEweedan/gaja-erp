...
const ProductDialog: React.FC<ProductDialogProps> = ({
  ...
  const filteredProducts = hasSearched
    ? pdata.filter((product) => {
        ...
        const typeSupplier =
          product.Fournisseur?.TYPE_SUPPLIER || product.TYPE_SUPPLIER || "";
  ...
  useEffect(() => {
    ...
    const uniqueIdFacts = Array.from(
      new Set(filteredProducts.map((p) => p.id_fact))
    ).slice(0, MAX_IMAGES);
        ...
        await Promise.all(
          idsToFetch.map(async (id_fact) => {
            try {
              ...
              if (
                picData &&
                typeof picData === "object" &&
                picData.type === "Buffer" &&
                Array.isArray(picData.data)
              ) {
          ...
          <Table>
            ...
            <TableBody>
              ...
              {filteredProducts.map((product) => (
                <TableRow
                  key={product.id_fact}
                  selected={selectedId === product.id_fact}
                >
