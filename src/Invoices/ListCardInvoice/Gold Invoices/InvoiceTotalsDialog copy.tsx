...
const InvoiceTotalsDialog: React.FC<InvoiceTotalsDialogProps> = ({
  ...
  const validateTotals = () => {
    // Compute final total after discount (numeric)
    const totalBase = Number(totals.total_remise_final) || 0;
    const finalAfterDiscountNumeric =
      discountType === "value"
        ? totalBase - (Number(totals.remise) || 0)
        : totalBase - totalBase * ((Number(totals.remise_per) || 0) / 100);
    ...
    if (Type_Supplier.toLowerCase().includes("gold")) {
      ...
      if (Math.abs(totalLyd - finalAfterDiscountNumeric) > 0.01) {
        alertList.push(
          `For gold type, Total Amount (LYD) must equal Final Total After Discount (LYD). Difference: ${(totalLyd - finalAfterDiscountNumeric).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
        );
      }
    }
  ...
  // When type is gold, keep `total_remise_final_lyd` in sync with the computed final total after discount
  React.useEffect(() => {
    if (!Type_Supplier.toLowerCase().includes("gold")) return;
    const totalBase = Number(totals.total_remise_final) || 0;
    const finalAfterDiscountNumeric =
      discountType === "value"
        ? totalBase - (Number(totals.remise) || 0)
    ...
    if (Math.abs(existing - finalAfterDiscountNumeric) > 0.005) {
      onChange("total_remise_final_lyd", Number(finalAfterDiscountNumeric));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [Type_Supplier, totals.total_remise_final, totals.remise, totals.remise_per, discountType]);
  ...
  return (
          ...
          >
            ...
            <Box sx={{ flex: "1 1 260px", minWidth: 220 }}>
              <TextField
                ...
                value={(() => {
                  // If gold, show the computed Final Total After Discount (LYD)
                  if (Type_Supplier.toLowerCase().includes("gold")) {
                    const total = Number(totals.total_remise_final) || 0;
                    const finalAfterDiscountNumeric =
                      discountType === "value"
                        ? total - (Number(totals.remise) || 0)
                        : total - total * ((Number(totals.remise_per) || 0) / 100);
                    return finalAfterDiscountNumeric.toLocaleString("en-US", {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    });
                  }
                  // Otherwise show the editable LYD total as-is
                  return (Number(totals.total_remise_final_lyd) || 0).toLocaleString("en-US", {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  });
                })()}
   ...
   {/* Comment field (Facture model: COMMENT) */}
