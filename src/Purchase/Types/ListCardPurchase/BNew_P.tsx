...
const BNew_p = (props: NewPProps) => {
  ...
  const validateForm = () => {
    ...
    if (!editPurchase.Original_Invoice)
      newErrors.Original_Invoice = "Reference purchase is required";
  ...
  const columnsFiltredTable = useMemo<MRT_ColumnDef<Purchase>[]>(
    ...
    [data]
  ); // No need for IS_OK in dependencies since it's part of row data
        ...
        <Box display={"inline-flex"} gap={2} justifyContent={"space-between"}>
          <Autocomplete
            ...
            getOptionLabel={(option) =>
              typeof option === "string" ? option : option.desig_art
            }
            value={
              Productsdata.find(
                (p) => p.desig_art === editPurchase.Design_art
              ) || null
            }
            onChange={(event, newValue) => {
              setEditPurchase((prev) => ({
