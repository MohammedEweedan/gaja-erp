...
const GOLD_COLORS = [
    'Yellow', 'White', 'Rose', 'Green', 'Red', 'Blue', 'Black', 'Grey', 'Champagne', 'Purple'
];
const STONE_COLORS = [
    'White', 'Black', 'Blue', 'Red', 'Green', 'Yellow', 'Pink', 'Purple', 'Orange', 'Brown', 'Grey', 'Champagne'
];
...
const GNew_p = (props: NewPProps) => {
    ...
    const validateForm = () => {
        ...
        if (editPurchase.Full_qty <= 0) newErrors.Full_qty = 'Full weight must be greater than 0';
    ...
    useEffect(() => {
        ...
        if (distribution) {
            setEditPurchase(prev => ({
                ...
                Original_Invoice: referencePurchase ? String(referencePurchase) : (distribution.PurchaseRef || ''),
                date_fact: new Date().toISOString().split('T')[0]
            }));
    ...
    useEffect(() => {
        if (brand && suppliers.length > 0) {
            const found = suppliers.find(s => s.id_client === brand || s.client_name.toLowerCase() === String(brand).toLowerCase());
            if (found) {
                setEditPurchase(prev => ({
                    ...prev,
                    Fournisseur: {
            ...
            >
                ...
                {distribution && (
                    ...
                    >
                        ...
                        >
                            Total: {Number(distribution.Weight).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}g&nbsp;|&nbsp;
                            Received: {totalWeight.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}g&nbsp;|&nbsp;
                            <span style={{ color: 'red' }}>
                                Rest: {(Number(distribution.Weight) - totalWeight).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}g
                            </span>
                        </Box>
                    </Box>
                )}
            ...
            <Box sx={{ display: 'grid', gap: 2, mt: 2 }}>
                    ...
                    <Autocomplete
                        ...
                        onInputChange={(event, inputValue) => {
                            if (!isBrandDisabled && inputValue) {
                                const found = suppliers.find(s => s.client_name.toLowerCase() === inputValue.toLowerCase());
                                if (found) {
                                    setEditPurchase(prev => ({
                                        ...prev,
                                        Fournisseur: {
                ...
                <Box display={'inline-flex'} gap={2} justifyContent={'space-between'}>
                    <TextField
                        ...
                        onChange={(e) =>
                            setEditPurchase(prev => ({ ...prev, Full_qty: parseFloat(e.target.value) }))
                        }
                    ...
                    <Autocomplete
                        ...
                        onChange={(event, newValue) =>
                            setEditPurchase(prev => ({ ...prev, Color_Rush: newValue || '' }))
                        }
                        renderInput={(params) => (
                            <TextField
                                {...params}
                    ...
                    <Autocomplete
                        ...
                        onChange={(event, newValue) =>
                            setEditPurchase(prev => ({ ...prev, Color_Gold: newValue || '' }))
                        }
                        renderInput={(params) => (
                            <TextField
                                {...params}
