...
const GPurchase = (props: Props) => {
  ...
  const [notReceivedDistributions, setNotReceivedDistributions] = useState<
    any[]
  >([]);
  const [receiveDistribution, setReceiveDistribution] = useState<any | null>(
    null
  );
  const [distributionToReceive, setDistributionToReceive] = useState<
    any | null
  >(null);
  const [receivedWeights, setReceivedWeights] = useState<
  ...
  const getLastMonthNbr = (purchases: Purchase[]) => {
    ...
    return purchases.reduce((count, purchase) => {
      ...
      if (
        ...
        !uniqueInvoices.has(purchase.num_fact)
      ) {
  ...
  const getPurchaseSummary = (purchaseNum: number) => {
    ...
    const isGold =
      firstItem?.TYPE_SUPPLIER?.toLowerCase().includes("gold") ||
      firstItem?.Fournisseur?.TYPE_SUPPLIER?.toLowerCase().includes("gold");

    if (isGold) {
      const totalQty = items.reduce((sum, item) => sum + (item.qty || 0), 0);
      const totalFullQty = items.reduce(
  ...
  const distinctByInvoice = useMemo(() => {
    ...
    const source = selectedSupplier
      ? groupedPurchases[selectedSupplier] || []
      : data;
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          showSnackbar(
            "Server error: Received HTML instead of data. Please contact support.",
            "error"
          );
          return;
        }
        ...
        showSnackbar(
          "Failed to fetch invoice data. Please check your connection or contact support.",
          "error"
        );
        return;
      }
    ...
    <Box p={2}>
      ...
      {!showPurchaseForm ? (
          ...
          {!selectedSupplier ? (
            <>
              {notReceivedDistributions.length > 0 && (
                ...
                >
                  <Box sx={{ mb: 1 }}>
                    New arrival has come and will be received soon. Please check
                        ...
                        >
                          ...
                          <li>
                            ...
                            {dist.PurchaseType?.toLowerCase().includes("gold")
                              ...
                              : dist.PurchaseType?.toLowerCase().includes(
                                    "diamond"
                                  )
                                ? (dist.purchaseD?.supplier?.client_name ??
                                  "N/A")
                                : dist.PurchaseType?.toLowerCase().includes(
                                      "watche"
                                    )
                                  ? (dist.purchaseW?.supplier?.client_name ??
                                    "N/A")
                                  : "N/A"}
                          </li>
                          <li>
                            <b>Created By:</b>{" "}
                            {dist.user?.name_user ?? dist.user?.name ?? "N/A"}
                          </li>
                          ...
                          {dist.PurchaseType?.toLowerCase().includes(
                            ...
                            <>
                              <li>
                                <b>Weight:</b>{" "}
                                {Number(dist.Weight).toLocaleString(undefined, {
                              ...
                              <li>
                                ...
                                <span style={{ color: "red" }}>
                                  {(
                                    Number(dist.Weight) -
                                    (receivedWeights[dist.distributionID] || 0)
                                  ).toLocaleString(undefined, {
                                    minimumFractionDigits: 2,
                                    maximumFractionDigits: 2,
                                  })}{" "}
        ...
        <>
          ...
          {Type === "boxes" && (
            <BNew_p num_fact={selectedInvoiceNum ?? undefined} />
          )}
        </>
      )}
    </Box>
