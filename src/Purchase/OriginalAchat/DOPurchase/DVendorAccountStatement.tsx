...
import {
    Box, Typography, Button, TextField, Autocomplete, Divider
} from '@mui/material';
...
const DVendorAccountStatement = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`${apiUrlVendors}/all`, { headers }).then(res => setVendors(res.data));
        axios.get(`${apiUrlBrands}/all`, { headers }).then(res => {
            const filtered = res.data.filter((s: Supplier) => s.TYPE_SUPPLIER?.toLowerCase().includes('diamond'));
            setBrands(filtered);
        });
    }, []);
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const computeNet = (r: RowItem) => {
                ...
                const discount = r.discount_by_vendor !== undefined ? Number(r.discount_by_vendor) : 0;
            ...
            const openingUSD = openingRows.reduce((sum, r) => sum + computeNet(r).netUSD, 0);
            const openingLYD = openingRows.reduce((sum, r) => sum + computeNet(r).netLYD, 0);
    ...
    const rowsWithBalance = useMemo(() => {
        ...
        return rows.map(row => {
            ...
            const netLYD = ((adjDebit - credit) * (Number(row.ExchangeRate) || 1) * (Number(row.ExchangeRateToLYD) || 1));
            balanceAmount += (adjDebit - credit);
            balanceUSD += netUSD;
            balanceLYD += netLYD;
            return { ...row, balanceUSD, balanceLYD, balanceAmount, netAmountUSD: netUSD, netAmountLYD: netLYD } as any;
        });
    ...
    const columns = useMemo<MRT_ColumnDef<any>[]>(() => [
        ...
        },
        {
            ...
            Cell: ({ row }) => {
                ...
                const discount = row.original.discount_by_vendor !== undefined ? Number(row.original.discount_by_vendor) : 0;
                ...
                return (
                    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                        {showDebit !== 0 && (
                            <span><span style={{ color: '#888', fontSize: 12 }}>Debit:</span> {showDebit.toLocaleString(undefined, { minimumFractionDigits: 2 })} {row.original.currency}</span>
                        )}
                        {showCredit !== 0 && (
                            <span><span style={{ color: '#888', fontSize: 12 }}>Credit:</span> {showCredit.toLocaleString(undefined, { minimumFractionDigits: 2 })} {row.original.currency}</span>
                        )}
        ...
        { accessorKey: 'netAmountUSD', header: 'Net Amount (USD)', size: 120, Cell: ({ cell }) => cell.getValue() ? Number(cell.getValue()).toLocaleString(undefined, { minimumFractionDigits: 2 }) + ' USD' : '' },
        ...
        <Box>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-start', mb: 2, flexWrap: 'wrap' }}>
                <Typography color="text.primary" variant="h5" sx={{ fontWeight: 'bold', mr: 2, mb: { xs: 1, sm: 0 } }}>
                    Diamond Vendor Account Statement
                </Typography>
                <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center', ml: 2 }}>
                    <TextField label="From" type="date" size="small" value={from} onChange={e => setFrom(e.target.value)} InputLabelProps={{ shrink: true }} sx={{ minWidth: 120, maxWidth: 140 }} />
                    <TextField label="To" type="date" size="small" value={to} onChange={e => setTo(e.target.value)} InputLabelProps={{ shrink: true }} sx={{ minWidth: 120, maxWidth: 140 }} />
                    <Autocomplete options={vendors} getOptionLabel={o => o.Client_Name} value={vendor} size="small" onChange={(_e, v) => setVendor(v)} renderInput={p => <TextField {...p} label="Vendor" size="small" sx={{ minWidth: 260, maxWidth: 340 }} />} sx={{ minWidth: 260, maxWidth: 340 }} />
                    <Autocomplete options={brands} getOptionLabel={o => o.client_name} value={brand} size="small" onChange={(_e, v) => setBrand(v)} renderInput={p => <TextField {...p} label="Brand" size="small" sx={{ minWidth: 260, maxWidth: 340 }} />} sx={{ minWidth: 260, maxWidth: 340 }} />
                    <Button variant="contained" color="info" onClick={fetchData}>Preview</Button>
                </Box>
            </Box>
            <Divider sx={{ mb: 2 }} />
            <Box>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 2, gap: 4 }}>
                    <Typography variant="subtitle1" sx={{ fontWeight: 'bold', color: '#1976d2' }}>
                        Final Balance (Amount): {rowsWithBalance.length > 0 ? Number((rowsWithBalance as any)[rowsWithBalance.length - 1].balanceAmount).toLocaleString(undefined, { minimumFractionDigits: 2 }) : '0.00'} {rowsWithBalance.length > 0 ? (rowsWithBalance as any)[rowsWithBalance.length - 1].currency : ''}
                    </Typography>
                    <Typography variant="subtitle1" sx={{ fontWeight: 'bold', color: '#1976d2' }}>
                        Final Balance (USD): {rowsWithBalance.length > 0 ? Number((rowsWithBalance as any)[rowsWithBalance.length - 1].balanceUSD).toLocaleString(undefined, { minimumFractionDigits: 2 }) : '0.00'} USD
                    </Typography>
                    <Typography variant="subtitle1" sx={{ fontWeight: 'bold', color: '#388e3c' }}>
                        Final Balance (LYD): {rowsWithBalance.length > 0 ? Number((rowsWithBalance as any)[rowsWithBalance.length - 1].balanceLYD).toLocaleString(undefined, { minimumFractionDigits: 2 }) : '0.00'} LYD
                    </Typography>
                </Box>
                <style>{`
