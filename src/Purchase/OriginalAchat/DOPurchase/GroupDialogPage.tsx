...
const GroupDialogPage: React.FC<GroupDialogPageProps> = ({ open, onClose, groupName, items, imageUrls }) => {
  ...
  const totalSalesPrice = React.useMemo(() => {
    ...
    return items.reduce((sum, item) => {
      // Use DistributionPurchase.OriginalAchatDiamond if available
        ...
        <DialogTitle>
          ...
          {items && items.length > 0 && (
            <span style={{ marginLeft: 16, fontWeight: 600, color: '#1976d2', fontSize: 16 }}>
              ...
              {(() => {
                // Try to get brand name from first item
                const first = items[0];
                let brand = '';
                if (first) {
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'row', gap: 2 }}>
            ...
            <Box sx={{ flex: 1 }}>
              {items && items.length > 0 ? (
                items.map((item, idx) => {
                  // Get latest image not containing 'group' or 'pic'
                  let imageUrl = '';
                  let arr: string[] = [];
                  if (imageUrls && item?.id_fact) {
                  ...
                  // Filter out images containing 'group' or 'pic' in filename
                  const filteredArr = arr.filter(u => !/group|pic/i.test(u.split('/').pop() || ''));
                  if (filteredArr.length) {
                    imageUrl = filteredArr[filteredArr.length - 1]; // latest
                  } else if (arr.length) {
                    imageUrl = arr[arr.length - 1]; // fallback to last
                  }

                  // Use getDiamondFromRow for correct field mapping
                  const diamond = item.DistributionPurchase?.OriginalAchatDiamond
                  ...
                  return (
                    <Box key={idx} sx={{ mb: 2, p: 1, border: '1px solid #eee', borderRadius: 1, display: 'flex', alignItems: 'flex-start', gap: 2, bgcolor: '#e5e5e5' }}>
                      ...
                      <Box sx={{ flex: 1 }}>
                        ...
                        {/* Inline details, bold labels, separated by spaces, like inventory */}
            ...
            {/* Image group column on the right: only for the first item */}
