...
import {
  ...
  Divider, Typography, Autocomplete
} from '@mui/material';
...
const OPurchase = () => {
  ...
  const [attachmentDialog, setAttachmentDialog] = useState<{ open: boolean, row: OPurchae | null }>({ open: false, row: null });
  const [attachment, setAttachment] = useState<File | null>(null);
  const [uploading, setUploading] = useState(false);
  const [sendingEmail, setSendingEmail] = useState(false);
  const [emailProgress, setEmailProgress] = useState(0);
  const [distributionDialog, setDistributionDialog] = useState<{ open: boolean, purchase: OPurchae | null }>({ open: false, purchase: null });
  const [distributions, setDistributions] = useState<DistributionPurchase[]>([]);
  const [newDistribution, setNewDistribution] = useState<{ ps: number; Weight: number; distributionDate: string, PurchaseType: string }>({ ps: 0, Weight: 0, distributionDate: new Date().toISOString().slice(0, 10), PurchaseType: 'Gold Purchase' });
  ...
  const [distributionErrors, setDistributionErrors] = useState<{ ps?: boolean; Weight?: boolean; distributionDate?: boolean }>({});
  ...
  const validateForm = () => {
    ...
    if (!editOPurchase?.FullWeight && editOPurchase?.FullWeight !== 0) newErrors.FullWeight = 'Required';
    if (!editOPurchase?.NetWeight && editOPurchase?.NetWeight !== 0) newErrors.NetWeight = 'Required';
    if (
      ...
      newErrors.FullWeight = 'Full Weight must be greater than or equal to Net Weight';
      newErrors.NetWeight = 'Net Weight must be less than or equal to Full Weight';
    }
    if (!editOPurchase?.Usr && editOPurchase?.Usr !== 0) newErrors.Usr = 'Required';
   ...
   // if (!editOPurchase?.PureWt) newErrors.PureWt = 'Required';
    ...
    if (!editOPurchase?.MakingStoneValue) newErrors.MakingStoneValue = 'Required';
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditMode) {
        await axios.put(`${apiUrl}/Update/${editOPurchase.id_achat}`, {
          ...editOPurchase,
  ...
  const handleExportExcel = () => {
    const headers = [
      "ID Achat", "Comment", "Date", "Full Weight", "Net Weight", "User", "Supplier",
      "Document No", "Stone Details", "Net Details", "Purity", "Pure Wt",
      "Making Stone Rate", "Making Stone Value", "Metal Value"
    ];
  ...
  const columns = useMemo<MRT_ColumnDef<OPurchae>[]>(() => [
    ...
    },
    {
      ...
      header: 'Supplier',
      size: 120,
      Cell: ({ row }) => suppliers.find(s => s.id_client === row.original.Brand)?.client_name || '',
    },
    {
   ...
   // { accessorKey: 'PureWt', header: 'Pure Wt', size: 80 },
    { accessorKey: 'MakingStoneRate', header: 'Making Stone Rate', size: 100 },
    { accessorKey: 'MakingStoneValue', header: 'Making Stone Value', size: 100 },
                    ...
                    try {
                      await axios.post('/Opurchases/send-approval', {
                        ...
                        purchaseInfo: {
                          ...
                          Supplier: suppliers.find(s => s.id_client === row.original.Brand)?.client_name || '',
    ...
    { accessorKey: 'MakingCharge', header: 'Making Charge', size: 100, Cell: ({ cell }) => formatAmount(cell.getValue<number>()) },
    { accessorKey: 'ShippingCharge', header: 'Shipping Charge', size: 100, Cell: ({ cell }) => formatAmount(cell.getValue<number>()) },
    { accessorKey: 'TravelExpesenes', header: 'Travel Expenses', size: 100, Cell: ({ cell }) => formatAmount(cell.getValue<number>()) },
    { accessorKey: 'cost_g', header: 'Cost (g)', size: 100, Cell: ({ cell }) => formatAmount(cell.getValue<number>()) },
    { accessorKey: 'Rate', header: 'Rate', size: 100, Cell: ({ cell }) => formatAmount(cell.getValue<number>()) },
    { accessorKey: 'cost_g_LYD', header: 'Cost (g) LYD', size: 100, Cell: ({ cell }) => formatAmount(cell.getValue<number>()) },
    {
      header: 'Edit Cost',
      id: 'journal',
      size: 80,
  ...
  const handleSaveJournal = async () => {
    ...
    const token = localStorage.getItem('token');
    try {
      await axios.put(`${apiUrl}/Update/${journalRow.id_achat}`, {
        ...journalRow,
        ...journalFields,
      }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setSnackbar({ open: true, message: 'Journal updated', severity: 'success' });
      await fetchData();
      setOpenJournalDialog(false);
      setJournalRow(null);
    } catch {
      setSnackbar({ open: true, message: 'Failed to update journal', severity: 'error' });
    }
  };
    ...
    <Box p={0.5}>
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog}>
        ...
        <DialogContent >
          ...
          >
            <TextField label="Comment" fullWidth required value={editOPurchase?.Comment_Achat || ''} onChange={e => setEditOPurchase({ ...editOPurchase!, Comment_Achat: e.target.value })} error={!!errors.Comment_Achat} helperText={errors.Comment_Achat} />
            <TextField label="Date" type="date" fullWidth required InputLabelProps={{ shrink: true }} value={editOPurchase?.Date_Achat || ''} onChange={e => setEditOPurchase({ ...editOPurchase!, Date_Achat: e.target.value })} error={!!errors.Date_Achat} helperText={errors.Date_Achat} />
            <TextField label="Full Weight" type="number" fullWidth required value={editOPurchase?.FullWeight || ''} onChange={e => setEditOPurchase({ ...editOPurchase!, FullWeight: Number(e.target.value) })} error={!!errors.FullWeight} helperText={errors.FullWeight} />
            <TextField label="Net Weight" type="number" fullWidth required value={editOPurchase?.NetWeight || ''} onChange={e => setEditOPurchase({ ...editOPurchase!, NetWeight: Number(e.target.value) })} error={!!errors.NetWeight} helperText={errors.NetWeight} />
            <Autocomplete
              id="supplier-select"
              sx={{ width: '100%' }}
              options={suppliers}
            ...
            />
            <TextField label="Document No" fullWidth required value={editOPurchase?.DocumentNo || ''} onChange={e => setEditOPurchase({ ...editOPurchase!, DocumentNo: e.target.value })} error={!!errors.DocumentNo} helperText={errors.DocumentNo} />
            <TextField label="Stone Details" fullWidth required value={editOPurchase?.Stone_Details || ''} onChange={e => setEditOPurchase({ ...editOPurchase!, Stone_Details: e.target.value })} error={!!errors.Stone_Details} helperText={errors.Stone_Details} />
            <TextField label="Net Details" fullWidth required value={editOPurchase?.Net_Details || ''} onChange={e => setEditOPurchase({ ...editOPurchase!, Net_Details: e.target.value })} error={!!errors.Net_Details} helperText={errors.Net_Details} />
            ...
            <TextField label="Making Stone Rate" fullWidth required value={editOPurchase?.MakingStoneRate || ''} onChange={e => setEditOPurchase({ ...editOPurchase!, MakingStoneRate: e.target.value })} error={!!errors.MakingStoneRate} helperText={errors.MakingStoneRate} />
            <TextField label="Making Stone Value" fullWidth required value={editOPurchase?.MakingStoneValue || ''} onChange={e => setEditOPurchase({ ...editOPurchase!, MakingStoneValue: e.target.value })} error={!!errors.MakingStoneValue} helperText={errors.MakingStoneValue} />
            <TextField label="Metal Value" fullWidth required value={editOPurchase?.MetalValue || ''} onChange={e => setEditOPurchase({ ...editOPurchase!, MetalValue: e.target.value })} error={!!errors.MetalValue} helperText={errors.MetalValue} />
          </Box>
        </DialogContent>
        <DialogActions >
      ...
      <Dialog open={distributionDialog.open} onClose={() => setDistributionDialog({ open: false, purchase: null })} maxWidth="sm" fullWidth>
        <DialogTitle>
        ...
        <DialogContent>
          ...
          <Box sx={{ mb: 2, display: 'flex', gap: 4, flexWrap: 'wrap' }}>
            <Typography variant="body2">
              <b>Net Weight:</b> {formatAmount(distributionDialog.purchase?.NetWeight ?? 0)} g
            </Typography>
            <Typography variant="body2">
              <b>Distributed:</b> {formatAmount(distributions.reduce((sum, d) => sum + (Number(d.Weight) || 0), 0))} g
            </Typography>
            <Typography variant="body2">
              <b>Difference:</b> {formatAmount((distributionDialog.purchase?.NetWeight ?? 0) - distributions.reduce((sum, d) => sum + (Number(d.Weight) || 0), 0))} g
            </Typography>
          </Box>
          {/* --- END NEW --- */}
          <Box sx={{ mb: 2 }}>
            <Typography variant="subtitle2" sx={{ mb: 1 }}>Add Distribution</Typography>
            <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
              ...
              <TextField
                ...
                onChange={e => setNewDistribution(nd => ({ ...nd, Weight: Number(e.target.value) }))}
                size="small"
                fullWidth
              ...
              <TextField
                ...
                helperText={distributionErrors.distributionDate ? "Required" : ""}
              />
              {/* Hide Add button if difference is 0 */}
              {((distributionDialog.purchase?.NetWeight ?? 0) - distributions.reduce((sum, d) => sum + (Number(d.Weight) || 0), 0)) > 0 && (
                <Button
                  variant="contained"
                  color="primary"
                  onClick={async () => {
                    ...
                    if (errors.ps || errors.Weight || errors.distributionDate) {
                      setSnackbar({
                        open: true,
                        message: 'All fields (Point of Sale, Weight, Date) are required.',
                        severity: 'warning'
                      });
                      return;
                    }
                    ...
                    const distributed = distributions.reduce((sum, d) => sum + (Number(d.Weight) || 0), 0);
                    const netWeight = distributionDialog.purchase?.NetWeight ?? 0;
                    if (distributed + Number(newDistribution.Weight) > netWeight) {
                      setSnackbar({
                        open: true,
                        message: 'Cannot distribute more than the Net Weight.',
                        severity: 'error'
                      });
                      setDistributionErrors(prev => ({ ...prev, Weight: true }));
                      return;
                    }
                    ...
                    try {
                      ...
                      setDistributions(res.data.filter(
                        (d: any) =>
                          d.PurchaseID === distributionDialog.purchase!.id_achat &&
                          d.PurchaseType === 'Gold Purchase'
                      ));
                      setNewDistribution({ ps: 0, Weight: 0, distributionDate: new Date().toISOString().slice(0, 10), PurchaseType: 'Gold Purchase' });
                      setDistributionErrors({});
                    } catch {
                      setSnackbar({ open: true, message: 'Failed to add distribution', severity: 'error' });
                    }
                  }}
                >
                  Add
          ...
          <Typography variant="subtitle2" sx={{ mb: 1 }}>Distributions</Typography>
            ...
            {loadingDistributions ? (
              ...
              <>
                ...
                <div id="print-receipt-area" style={{ display: 'none' }}>
                  <div style={{ width: 160, height: 80, textAlign: 'left', marginBottom: 8, overflow: 'hidden' }}>
                    <Logo />
                  </div>
                  <h2 style={{ textAlign: 'center' }}>Distribution Transaction</h2>
                  <p><b>Purchase ID:</b> {distributionDialog.purchase?.id_achat}</p>
                  <table>
                    <thead>
                      <tr>
                        <th>PS</th>
                    ...
                    <tbody>
                      {distributions.map((dist) => (
                        <tr key={dist.distributionID}>
                          <td>{psList.find(ps => ps.Id_point === dist.ps)?.name_point || dist.ps}</td>
                  ...
                  <div style={{ marginTop: 32, fontSize: 14, textAlign: 'center' }}>
                    <b>ملاحظة هامة:</b>
                    <div>
                      يجب على محاسب الفرع أن يدخل إلى النظام والقيام باستلام هذا الطرد ومطابقته، لأنها حالياً تعتبر في عهدته.<br />
                      كما يجب إضافة توقيع المستلم.
                    </div>
                    <hr style={{ margin: '16px 0' }} />
                    <b>Important Note:</b>
                    <div>
                      The branch accountant must log into the system to receive and verify this package, as it is currently under their responsibility.<br />
                      The recipient's signature must also be added.
                    </div>
                    <div style={{ marginTop: 24, textAlign: 'left' }}>
                      <b>توقيع المستلم / Recipient Signature:</b>
                      <div style={{ borderBottom: '1px solid #888', width: 220, height: 32, marginTop: 8 }}></div>
                    </div>
                  </div>
                </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                      ...
                      <tr key={dist.distributionID}>
                        ...
                        <td>
                          ...
                          <Button
                            ...
                            onClick={() => {
                              ...
                              if (printWindow && printContents) {
                                printWindow.document.write('<html><head><title>Distribution Receipt</title>');
                                printWindow.document.write('<style>body{font-family:sans-serif;} table{width:100%;border-collapse:collapse;} th,td{border:1px solid #ccc;padding:8px;text-align:left;} th{background:#eee;}</style>');
                          ...
                          >
                            ...
                            <h2 style={{ textAlign: 'center' }}>Transfer No : {dist.distributionID}</h2>
                            <table>
                              <tbody>
                                <tr>
                                  <th>PS</th>
                                ...
                                <tr>
                                  <th>Purchase ID</th>
                                  <td>{distributionDialog.purchase?.id_achat}</td>
                                </tr>
                              </tbody>
                            </table>
                            {/* Clarification and signature area */}
                    ...
                    {distributions.length === 0 && (
                      <tr>
                        <td colSpan={4} style={{ textAlign: 'center', color: '#888' }}>No distributions yet.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
        ...
        <DialogActions>
          ...
          <Button onClick={() => setDistributionDialog({ open: false, purchase: null })}
            variant="outlined"
            color="inherit"
            ...
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setDistributions(prev => prev.filter(d => d.distributionID !== pendingDeleteDist.distributionID));
                } catch {
      ...
      <Backdrop
        ...
        open={sendingEmail}
      >
        <Box sx={{ width: 300, bgcolor: 'background.paper', p: 4, borderRadius: 2, boxShadow: 3, textAlign: 'center' }}>
      ...
      >
        ...
        <DialogContent sx={{ minWidth: 600 }}>
          <Box sx={{ display: 'flex', flexDirection: 'row', gap: 2, mt: 1, minWidth: 420 }}>


            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, ml: 2, mt: 1, minWidth: 280 }}>
              <TextField
                label="Making Charge"
                type="number"
                value={journalFields.MakingCharge}
                onChange={e => setJournalFields(f => ({ ...f, MakingCharge: Number(e.target.value) }))}
                fullWidth
              />
              <TextField
                ...
                onChange={e => setJournalFields(f => ({ ...f, ShippingCharge: Number(e.target.value) }))}
                fullWidth
              />
              <TextField
                ...
                onChange={e => setJournalFields(f => ({ ...f, TravelExpesenes: Number(e.target.value) }))}
                fullWidth
              />
              <TextField
                label="Cost (g) [USD]"
              ...
              <TextField
                ...
                onChange={e => setJournalFields(f => ({ ...f, ounceCost: Number(e.target.value) }))}
                fullWidth
              />
            ...
            <Box sx={{
              ...
              <Typography variant="body2" sx={{ mt: 1 }}>
                <b>Total (USD):</b> {formatAmount((journalRow?.NetWeight ?? 0) * (journalFields.cost_g || 0))}
              </Typography>
              <Typography variant="body2">
                <b>Total (LYD):</b> {formatAmount((journalRow?.NetWeight ?? 0) * (journalFields.cost_g_LYD || 0))}
              </Typography>
              ...
              {[
                ...
                { label: 'Shipping Charge', value: journalFields.ShippingCharge },
                { label: 'Travel Expenses', value: journalFields.TravelExpesenes }
              ].map((item, idx) => (
                <Typography variant="body2" key={item.label} sx={{ ml: 1 }}>
                  {item.label}: {formatAmount(item.value)} USD × {formatAmount(journalFields.Rate)} = {formatAmount(item.value * (journalFields.Rate || 0))} LYD
                </Typography>
              ))}
              <Divider sx={{ my: 1, borderBottomWidth: 2 }} />
              <Typography variant="body2" sx={{ fontWeight: 'bold', color: 'primary.main' }}>
                Grand Total (USD): {formatAmount(
              ...
              <Typography variant="body2" sx={{ fontWeight: 'bold', color: 'primary.main' }}>
                Grand Total (LYD): {formatAmount(
                  (journalRow?.NetWeight ?? 0) * (journalFields.cost_g_LYD || 0) +
                  (journalFields.MakingCharge + journalFields.ShippingCharge + journalFields.TravelExpesenes) * (journalFields.Rate || 0)
                )}
              </Typography>

              <Divider sx={{ my: 1, borderBottomWidth: 2 }} />

              <Button sx={{ position:'revert'}} variant='contained' onClick={() => { setOpenJournalDialog(false); setJournalRow(null); }} color="success">
                Generate Journal
              </Button>
        ...
        <DialogActions sx={{pr:4}}>
          ...
          <Button variant='contained' onClick={() => setOpenJournalDialog(false)} color="secondary">
            Cancel
          </Button>
          <Button variant='contained' onClick={handleSaveJournal} color="primary">
            Save
          </Button>
