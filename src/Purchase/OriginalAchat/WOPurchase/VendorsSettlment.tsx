...
const VendorsSettlment = () => {
    ...
    const [editSettlement, setEditSettlement] = useState<SupplierSettlement | null>(null);
    ...
    const handleSave = async () => {
        ...
        try {
            if (isEditMode) {
                await axios.put(`${apiUrl}/Update/${editSettlement.id_settlement}`, editSettlement, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            } else {
            ...
            await fetchData();
            handleCloseDialog();
            setSnackbar({ open: true, message: isEditMode ? 'Changes saved successfully' : 'Saved successfully', severity: 'success' });
        } catch (error: any) {
    ...
    const columns = useMemo<MRT_ColumnDef<SupplierSettlement>[]>(() => [
            ...
            Cell: ({ row }) => {
                ...
                return (
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                        <span style={{ fontWeight: 500 }}><span style={{color:'#888', fontSize:12}}>Ref. number:</span> {ref}</span>
                        <span style={{ color: '#666', fontSize: 13 }}><span style={{color:'#888', fontSize:12}}>Date:</span> {dateStr}</span>
                    </Box>
                );
            },
        },
        {
            ...
            size: 160,
            Cell: ({ row }) => {
                const supplier = suppliers.find(s => s.id_client === row.original.client);
                const vendor = vendors.find(v => v.ExtraClient_ID === row.original.Brand);
                return (
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                        <span style={{ fontWeight: 500 }}><span style={{color:'#888', fontSize:12}}>Brand:</span> {supplier ? supplier.client_name : row.original.client || '-'}</span>
                        <span style={{ fontWeight: 500 }}><span style={{color:'#888', fontSize:12}}>Vendor:</span> {vendor ? vendor.Client_Name : row.original.Brand || '-'}</span>
                    </Box>
                );
            },
        },
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                        {debit !== 0 && (
                            <span style={{ fontWeight: 500 }}>
                                <span style={{color:'#888', fontSize:12}}>Debit:</span> {debit.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} {currency}
                            </span>
                        )}
                        {credit !== 0 && (
                            <span style={{ fontWeight: 500 }}>
                ...
                return (
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                        <Box sx={{ display: 'flex', flexDirection: 'row', gap: 1 }}>
                            ...
                            <span style={{fontWeight: 500}}>{!isNaN(netUSD) ? netUSD.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : ''} USD</span>
                        </Box>
                        <Box sx={{ display: 'flex', flexDirection: 'row', gap: 1 }}>
                            ...
                            <span style={{fontWeight: 500}}>{!isNaN(netLYD) ? netLYD.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : ''} LYD</span>
                        </Box>
                    </Box>
                );
            },
       ...
       
        {
            ...
            size: 120,
            Cell: ({ cell }) => {
                const vendor = vendors.find(v => v.ExtraClient_ID === cell.getValue<number>());
    ...
    return (
        <Box p={0.5}>
                ...
                <DialogContent>
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 2 }}>
                        ...
                        <Autocomplete
                            ...
                            value={vendorsPby.find(v => v.ExtraClient_ID === editSettlement?.Paidby) || null}
                            ...
                            renderInput={(params) => (
                                <TextField
                                    {...params}
                                    label={<span style={{ color: errors.Paidby ? '#d32f2f' : undefined }}>Paid By</span>}
                                    error={!!errors.Paidby}
                                    sx={!!errors.Paidby ? { '& .MuiOutlinedInput-root': { '& fieldset': { borderColor: '#d32f2f', borderWidth: 2 } } } : {}}
                        ...
                        <Box sx={{ display: 'flex', gap: 2, border: '1px solid #e0e0e0', borderRadius: 2, p: 2, mb: 2, flexWrap: 'wrap' }}>
                            ...
                            <Autocomplete
                                ...
                                renderInput={(params) => (
                                    <TextField
                                        {...params}
                                        label={<span style={{ color: errors.Brand ? '#d32f2f' : undefined }}>Vendor</span>}
                                        error={!!errors.Brand}
                                        sx={!!errors.Brand ? { '& .MuiOutlinedInput-root': { '& fieldset': { borderColor: '#d32f2f', borderWidth: 2 } } } : {}}
                            ...
                            <Autocomplete
                                ...
                                renderInput={(params) => (
                                    <TextField
                                        {...params}
                                        label={<span style={{ color: errors.client ? '#d32f2f' : undefined }}>Supplier</span>}
                                        error={!!errors.client}
                                        sx={!!errors.client ? { '& .MuiOutlinedInput-root': { '& fieldset': { borderColor: '#d32f2f', borderWidth: 2 } } } : {}}
                        ...
                        <Box sx={{ display: 'flex', gap: 2, border: '1px solid #e0e0e0', borderRadius: 2, p: 2, mb: 2, flexWrap: 'wrap' }}>
                            <TextField
                                ...
                                type="number"
                                fullWidth
                                error={!!errors.Debit_Gold}
                                helperText={errors.Debit_Gold}
                            ...
                            <TextField
                                ...
                                required
                                type="number"
                                error={!!errors.Credit_Gold}
                                helperText={errors.Credit_Gold}
                                fullWidth
            ...
            <Dialog open={deleteDialog.open} onClose={() => setDeleteDialog({ open: false, row: null })}>
                ...
                <DialogContent>
                    Are you sure you want to delete payment #{deleteDialog.row?.id_settlement}?
                </DialogContent>
                <DialogActions>
                    <Button onClick={() => setDeleteDialog({ open: false, row: null })} color="secondary">Cancel</Button>
