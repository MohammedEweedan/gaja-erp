...
const ListPurchase = () => {
  ...
  const table = useMaterialReactTable({
    ...
    muiTableBodyRowProps: ({ row }) => ({
      sx: {
        backgroundColor: themeMode === localStorage.getItem('themeMode')
          ? (row.index % 2 === 0 ? '#2a2a2a' : '#1e1e1e')
          : (row.index % 2 === 0 ? '#fafafa' : '#ffffff'),
        '&:hover': {
          backgroundColor: themeMode === localStorage.getItem('themeMode') ? '#333' : '#eaeaea',
        },
      },
    }),
  });
