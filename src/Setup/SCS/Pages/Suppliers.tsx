...
import {
  ...
  Divider, Typography, createTheme, ThemeProvider, Card, CardContent,
  CardActionArea, Select, MenuItem, InputLabel, FormControl, CircularProgress, Alert
} from '@mui/material';
...
const Suppliers = () => {
  ...
  const handleBulkApply = async () => {
    ...
    const updates = filteredData.map(async (supplier) => {
      const updatedSupplier = {
        ...supplier,
        ...(bulkMaxDiscount !== '' ? { Percentage_Diamond: Number(bulkMaxDiscount) } : {}),
      };
      return axios.put(`${apiUrl}/Update/${supplier.id_client}`, updatedSupplier, {
    ...
    try {
      ...
      if (selectedType) {
        setFilteredData((prev) => {
          ...
          return latest.filter((s: Supplier) => s.TYPE_SUPPLIER === selectedType);
        });
      }
      setBulkMaxDiscount('');
    } catch (error: any) {
  ...
  const validateForm = () => {
    const newErrors: any = {};
    if (!editSupplier?.client_name) newErrors.client_name = 'Brand Name is required';
    ...
    if (
      ...
      newErrors.Percentage_Diamond = 'Maximum Discount % must be between 0 and 100';
    }
    setErrors(newErrors);
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditMode) {
        await axios.put(`${apiUrl}/Update/${editSupplier.id_client}`, editSupplier, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } else {
  ...
  const handleExportExcel = () => {
    const headers = ["ID", "Brand Name", "Phone", "Address", "Initial Balance",
      "Brand Code", "Diamond State", "Type", "Gold Price", "Diamond %", "Gold Sales"];
    const rows = filteredData.map(s => [
      s.id_client, s.client_name, s.tel_client, s.Adresse, s.Solde_initial,
      s.code_supplier, s.STATE_GOL_DIAMON ? "Yes" : "No", s.TYPE_SUPPLIER,
      s.Price_G_Gold, s.Percentage_Diamond, s.Price_G_Gold_Sales
    ]);
        ...
        {!selectedMainName ? (
          ...
          <>
            <Box display="flex" alignItems="center" gap={1} mb={2}>
              ...
              <Typography variant="h5" fontWeight="bold">Brand Types in {selectedMainName}</Typography>
            </Box>
            <Box display="flex" flexWrap="wrap" justifyContent="space-between"  gap={2}>
              {filteredItemTypes.map((type, i) => (
                <Box key={i} sx={{ width: { xs: '100%', sm: '50%', md: '18%' } }}>
                  <Card>
                    <CardActionArea onClick={() => handleTypeSelect(type.desig_unit)}>
                      <CardContent sx={{ textAlign: 'center' }}>
                        <CategoryIcon sx={{ fontSize: 40 }} color="disabled" />
          ...
          <>
            ...
            <Box mb={2}>
              <Alert severity="warning" sx={{ mb: 1 }}>
                <b>Clarification:</b> This field allows you to set the <b>maximum discount percentage</b> that can be applied to all brands of the selected type. For example, entering <b>10</b> will set the maximum allowed discount to <b>10%</b> for every brand in this group. This does not apply the discount directly to any transaction, but rather sets a limit for future sales or operations involving these brands. Please ensure you enter a value between 0 and 100. This action will update the "Maximum Discount %" column for all brands of the selected type.
              </Alert>
        ...
        <Dialog open={openDialog} onClose={handleCloseDialog}>
          ...
          <DialogContent>
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, mt: 2 }}>
              <TextField label="Brand Name" fullWidth value={editSupplier?.client_name || ''} onChange={(e) => setEditSupplier({ ...editSupplier!, client_name: e.target.value })} error={!!errors.client_name} helperText={errors.client_name} />
              <TextField label="Phone" fullWidth value={editSupplier?.tel_client || ''} onChange={(e) => setEditSupplier({ ...editSupplier!, tel_client: e.target.value })} error={!!errors.tel_client} helperText={errors.tel_client} />
              <TextField label="Address" fullWidth value={editSupplier?.Adresse || ''} onChange={(e) => setEditSupplier({ ...editSupplier!, Adresse: e.target.value })} />
              <TextField label="Brand Code" fullWidth value={editSupplier?.code_supplier || ''} onChange={(e) => setEditSupplier({ ...editSupplier!, code_supplier: e.target.value })} />
              <TextField
                ...
                onChange={e => {
                  ...
                  if (/^\d{0,3}$/.test(val)) {
                    let num = Number(val);
                    if (val === '') {
                      setEditSupplier({ ...editSupplier!, Percentage_Diamond: '' as any });
                    } else if (num >= 0 && num <= 100) {
                      setEditSupplier({ ...editSupplier!, Percentage_Diamond: num });
                    }
                  }
                }}
                inputProps={{ min: 0, max: 100 }}
              ...
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) =>
                    setEditSupplier({ ...editSupplier!, TYPE_SUPPLIER: e.target.value })
                  }
                  label="Type"
                >
