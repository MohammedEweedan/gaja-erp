...
import {
  ...
  Divider, Typography, createTheme, ThemeProvider, Switch, FormControlLabel
} from '@mui/material';
...
const Sm = () => {
  ...
  const validateForm = () => {
    const newErrors: any = {};
    if (!editBoxe?.SourceMarketing) newErrors.SourceMarketing = 'Designation is required';
    setErrors(newErrors);
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditMode) {
        await axios.put(`${apiUrl}/Update/${editBoxe.id_SourceMark}`, editBoxe, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } else {
