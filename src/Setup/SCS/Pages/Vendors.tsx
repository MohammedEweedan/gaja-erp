...
const Vendors = () => {
  ...
  const validateForm = () => {
    const newErrors: any = {};
    if (!editVendor?.Client_Name) newErrors.Client_Name = 'Client Name is required';
    setErrors(newErrors);
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditMode) {
        await axios.put(`${apiUrl}/Update/${editVendor.ExtraClient_ID}`, editVendor, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } else {
  ...
  const handleExportExcel = () => {
    const headers = ["ExtraClient ID", "Client Name", "Initial Sold Money", "Initial Sold Gold"];
  ...
  const columns = useMemo<MRT_ColumnDef<Vendor>[]>(() => [
    ...
    { accessorKey: 'Intial_Sold_Money', header: 'Initial Sold Money', size: 120 },
    { accessorKey: 'Intial_Sold_Gold', header: 'Initial Sold Gold', size: 120 },
    {
      header: 'Actions',
      id: 'actions',
      size: 100,
          ...
          <DialogContent  >
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, mt: 2 }}>
              ...
              <TextField
                ...
                onChange={(e) => setEditVendor({ ...editVendor!, Client_Name: e.target.value })}
              ...
              <TextField
                ...
                onChange={(e) => setEditVendor({ ...editVendor!, Intial_Sold_Money: Number(e.target.value) })}
              />
              <TextField
                ...
                onChange={(e) => setEditVendor({ ...editVendor!, Intial_Sold_Gold: Number(e.target.value) })}
              />
            </Box>
          </DialogContent>
          <DialogActions  >
