...
type Customer = {
  ...
  Adresse: string;
  email: string;
  usr?: string; // username who created this customer
};
...
const Customers = () => {
  ...
  const validateForm = () => {
    const newErrors: any = {};
    if (!editCustomer?.client_name)
      newErrors.client_name = "Client Name is required";
    if (!editCustomer?.tel_client)
      newErrors.tel_client = "Phone Number is required";
    setErrors(newErrors);
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditMode) {
        await axios.put(
          `${apiUrl}/Update/${editCustomer.id_client}`,
          editCustomer,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        ...
        // set current username as creator (usr) before creating
        let currentUser = "";
        const userStr = localStorage.getItem("user");
        if (userStr) {
          try {
            const userObj = JSON.parse(userStr);
            currentUser = userObj.name_user ?? userObj.name ?? "";
          } catch {
            currentUser = localStorage.getItem("name_user") || "";
          }
        }
        const payload = { ...editCustomer, usr: currentUser };
  ...
  const columns = useMemo<MRT_ColumnDef<Customer>[]>(
    () => [
      ...
      { accessorKey: "usr", header: "Created By", size: 140 },
      {
        header: "Actions",
        id: "actions",
        size: 100,
