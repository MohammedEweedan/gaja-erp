...
const Products = () => {
  ...
  const validateForm = () => {
    const newErrors: any = {};
    if (!editProduct?.desig_famille)
      newErrors.desig_famille = "Designation is required";
    setErrors(newErrors);
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditMode) {
        await axios.put(
          `${apiUrl}/Update/${editProduct.id_famille}`,
          editProduct,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
