...
const WInventory = (props: Props) => {
  ...
  const columns = useMemo<MRT_ColumnDef<InventoryItem>[]>(
    ...
    [images, imageUrls, carouselIndex]
  );
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((row) => {
      ...
      const minOk = min === null || priceVal >= min;
      const maxOk = max === null || priceVal <= max;
      return brandOk && nameOk && refOk && minOk && maxOk;
    });
  }, [data, brandFilter, productName, refCode, costMin, costMax]);
    ...
    const tableRows = await Promise.all(
      data.map(async (row) => {
        ...
        if (watch) {
          details =
            (watch.id_achat
              ? `| system Original ref.: ${watch.id_achat} `
              : "") +
            (watch.reference_number
